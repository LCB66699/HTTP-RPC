// Minimal MinIO/S3 client 鈥?implements PutObject and PresignedGetUrl
...
// Authority (host or host:port) for SigV4 鈥?must match the Host header the
...
struct Client {
    ...
    std::string public_url;  // external base URL for presigned links, e.g.
                             // "http://localhost:9000"
    ...
    bool PutObject(const std::string &key, const std::string &data,
        ...
        std::string signing_key =
            hmac256(hmac256(hmac256(hmac256("AWS4" + secret_key, date8), region), "s3"), "aws4_request");
        std::string signature = hmac256hex(signing_key, string_to_sign);

        std::string auth = "AWS4-HMAC-SHA256 Credential=" + access_key + "/" + scope +
        ...
        httplib::Headers headers = {
            {"Host", host}, {"x-amz-date", dt16}, {"x-amz-content-sha256", payload_hash}, {"Authorization", auth}};
    ...
    // Authenticated GET object (SigV4) 鈥?reads from internal `endpoint` only.
    // Used so downloads can go gateway 鈫?gRPC 鈫?MinIO without browser 鈫?
    // :9000/:443.
    bool GetObject(const std::string &key, std::string &out_body) const {
        ...
        std::string host = endpoint;
        std::string uri = "/" + bucket + "/" + url_encode_path(key);
        // Empty request body 鈥?standard hex(SHA256("")) used by AWS SigV4 for GET.
    ...
    // DELETE object 鈥?S3 SigV4 signed, same pattern as GetObject.
    ...
    std::string PresignedGetUrl(const std::string &key, int expires = 3600) const {
        ...
        std::string query =
            ...
            "&X-Amz-Credential=" +
            url_encode(credential) + "&X-Amz-Date=" + dt16 + "&X-Amz-Expires=" + std::to_string(expires) +
        ...
        // Use public_url so the presigned link works from outside the Docker
        // network.
