...
std::string CallLogger::NowString() const {
  ...
  oss << (lt->tm_year + 1900) << "-" << pad2(lt->tm_mon + 1) << "-"
      << pad2(lt->tm_mday) << " " << pad2(lt->tm_hour) << ":"
      << pad2(lt->tm_min) << ":" << pad2(lt->tm_sec) << "."
...
std::vector<CallEntry>
CallLogger::GetHistory(int limit, int offset, const std::string &service_filter,
