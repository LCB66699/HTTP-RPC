...
// 调用 Auth 服务验证调用者身份
bool SpreadsheetServiceImpl::ValidateCaller(grpc::ServerContext *ctx,
                                            int64_t user_id,
  ...
  vu_ctx.set_deadline(std::chrono::system_clock::now() +
...
static std::string ListCacheKey(int64_t user_id, int64_t version, int page,
                                int page_size) {
  std::string key =
      "u:" + std::to_string(user_id) + ":sheets:v" + std::to_string(version);
  if (page_size > 0)
...
grpc::Status SpreadsheetServiceImpl::CreateSpreadsheet(
  ...
  if (minio_ && minio_->IsConfigured()) {
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(
                  ...
                  .count();
    storage_key =
        std::to_string(req->user_id()) + "/" + std::to_string(us) + ".json";
    nlohmann::json content;
    content["headers"] = nlohmann::json::parse(
        req->headers_json().empty() ? "[]" : req->headers_json());
    content["data"] = nlohmann::json::parse(
        req->data_json().empty() ? "[]" : req->data_json());
  ...
  int64_t id = 0;
  bool ok = db_->CreateSpreadsheet(
      req->user_id(), username, req->name(), req->description(),
      req->headers_json(), req->data_json(), id, req->idempotency_key());
  ...
  if (redis_ && redis_->IsConnected()) {
    ...
    if (slog_)
      LOG_DEBUG(*slog_,
                "Create id=" + std::to_string(id) +
                    " INCR version for uid=" + std::to_string(req->user_id()));
  }
...
grpc::Status
SpreadsheetServiceImpl::GetSpreadsheet(grpc::ServerContext *context,
  ...
  int64_t owner_uid = 0;
  if (!db_ || !db_->GetSpreadsheetOwner(req->id(), owner_uid) ||
  ...
  const std::string cache_key =
      "u:" + std::to_string(req_uid) + ":sheet:" + std::to_string(req->id());
  const std::string ts_key = cache_key + ":ts";
  const std::string lock_key = "lock:u:" + std::to_string(req_uid) +
                               ":sheet:" + std::to_string(req->id());
  ...
  auto async_refresh = [this, kNullMarker,
                        &fillFromMinIO](int64_t id, int64_t uid, std::string ck,
                                        std::string tk, std::string lk) {
    SpreadsheetRow row;
    if (db_ && db_->GetSpreadsheet(id, uid, row)) {
      ...
      if (fresh.SerializeToString(&ser) && redis_ && redis_->IsConnected()) {
        ...
        if (slog_)
          LOG_DEBUG(*slog_, "Get id=" + std::to_string(id) +
                                " ASYNC-REFRESHED key=" + ck);
      }
      ...
      redis_->SetJSON(tk, std::to_string(std::time(nullptr)),
                      RedisClient::JitteredTTL(NULL_TTL, 30));
      if (slog_)
        LOG_DEBUG(*slog_, "Get id=" + std::to_string(id) +
                              " ASYNC-NULL-CACHED key=" + ck);
    }
    if (redis_ && redis_->IsConnected()) {
      redis_->DeleteKey(lk);
    }
    ...
    if (redis_->GetJSON(cache_key, cached)) {
      ...
      if (resp->ParseFromString(cached)) {
        ...
        if (need_refresh && redis_->SetNX(lock_key, "1", LOCK_TTL)) {
          if (slog_)
            LOG_DEBUG(*slog_,
                      "Get id=" + std::to_string(req->id()) +
                          " STALE (logical-expire), async refresh triggered");
          std::thread([this, async_refresh, id = req->id(), uid = req_uid,
                       ck = cache_key, tk = ts_key, lk = lock_key]() {
      ...
      if (slog_)
        LOG_ERROR(*slog_, "Get id=" + std::to_string(req->id()) +
                              " ParseFromString FAILED key=" + cache_key);
    } else {
      if (slog_)
        LOG_DEBUG(*slog_, "Get id=" + std::to_string(req->id()) +
                              " MISS key=" + cache_key);
  ...
  if (!db_->GetSpreadsheet(req->id(), req_uid, row)) {
    ...
    if (redis_ && redis_->IsConnected()) {
      redis_->SetJSON(cache_key, kNullMarker,
                      RedisClient::JitteredTTL(NULL_TTL, 30));
      redis_->SetJSON(ts_key, std::to_string(std::time(nullptr)),
                      RedisClient::JitteredTTL(NULL_TTL, 30));
      if (slog_)
        LOG_DEBUG(*slog_, "Get id=" + std::to_string(req->id()) +
                              " NULL-CACHED key=" + cache_key);
    }
    resp->set_success(false);
    resp->set_error("Not found");
    if (logger_) {
...
grpc::Status SpreadsheetServiceImpl::ListSpreadsheets(
  ...
  if (redis_ && redis_->IsConnected()) {
    ...
    if (redis_->GetJSON(cache_key, cached)) {
      if (resp->ParseFromString(cached)) {
        if (slog_)
          LOG_DEBUG(*slog_, "List uid=" + std::to_string(uid) + " v" +
      ...
      if (slog_)
        LOG_ERROR(*slog_, "List uid=" + std::to_string(uid) +
                              " ParseFromString FAILED key=" + cache_key);
    } else {
      if (slog_)
        LOG_DEBUG(*slog_, "List uid=" + std::to_string(uid) + " v" +
    ...
    if (resp->SerializeToString(&serialized)) {
      if (redis_->SetJSON(cache_key, serialized, 120)) {
        if (slog_)
          LOG_DEBUG(*slog_, "List uid=" + std::to_string(uid) + " v" +
                                std::to_string(version) +
                                " POPULATED key=" + cache_key + " size=" +
                                std::to_string(serialized.size()) + " ttl=120");
      }
    }
  }
...
grpc::Status SpreadsheetServiceImpl::UpdateSpreadsheet(
  ...
  for (int attempt = 0; attempt < MAX_RETRIES; ++attempt) {
    int64_t owner_uid = 0;
    int version = 0;
    if (!db_->GetSpreadsheetOwner(req->id(), owner_uid, &version) ||
        owner_uid != req->user_id()) {
      resp->set_success(false);
    ...
    bool ok = db_->UpdateSpreadsheet(req->id(), req->user_id(), req->name(),
                                     req->description(), req->headers_json(),
                                     req->data_json(), version);

    if (ok) {
      if (redis_ && redis_->IsConnected()) {
        redis_->DeleteKey("u:" + std::to_string(req->user_id()) +
                          ":sheet:" + std::to_string(req->id()));
        redis_->DeleteKey("u:" + std::to_string(req->user_id()) +
                          ":sheet:" + std::to_string(req->id()) + ":ts");
        redis_->Increment(VersionKey(req->user_id()));
        if (slog_)
          LOG_DEBUG(*slog_, "Update id=" + std::to_string(req->id()) +
                                " INVALIDATED + INCR version uid=" +
                                std::to_string(req->user_id()));
      }
      ...
      if (slog_)
        LOG_INFO(*slog_,
                 "Updated id=" + std::to_string(req->id()) + " by " + username);
      return grpc::Status::OK;
    }

    if (slog_)
      LOG_WARN(*slog_, "Update id=" + std::to_string(req->id()) +
...
grpc::Status SpreadsheetServiceImpl::DeleteSpreadsheet(
  ...
  int64_t owner_uid = 0;
  if (!db_->GetSpreadsheetOwner(req->id(), owner_uid) ||
      owner_uid != req->user_id()) {
    resp->set_success(false);
    resp->set_error("Not found or permission denied");
    return grpc::Status::OK;
  ...
  if (slog_)
    LOG_INFO(*slog_,
             "Deleted id=" + std::to_string(req->id()) + " by " + username);
  return grpc::Status::OK;
}
