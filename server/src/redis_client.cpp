...
RedisClient::RedisClient(const std::vector<std::string> &cluster_seeds,
    ...
    : cluster_seeds_(cluster_seeds), password_(password),
      pool_size_(pool_size) {}

RedisClient::~RedisClient() { cluster_.reset(); }
...
bool RedisClient::PushCallEntry(const std::string &json_entry,
  ...
  try {
    if (!username.empty()) {
      ...
      cluster_->expire(user_key,
                       std::chrono::seconds(CALL_HISTORY_TTL_SECONDS));
      cluster_->sadd("call_history:users", username);
      cluster_->expire("call_history:users",
                       std::chrono::seconds(CALL_HISTORY_TTL_SECONDS));
    }
    return true;
  } catch (const sw::redis::Error &e) {
...
RedisClient::GetCallEntries(int limit, int offset,
  ...
  try {
    ...
    cluster_->lrange(key, offset, offset + limit - 1,
...
bool RedisClient::SetJSON(const std::string &key, const std::string &value,
                          int ttl) {
  if (!cluster_)
    return false;
  try {
...
bool RedisClient::HSetJSON(const std::string &key, const std::string &field,
...
void RedisClient::SubscribeStandalone(const std::string &channel,
                                      SubCallback cb) {
  ...
  std::thread([this, channel, cb = std::move(cb)]() {
    try {
      ...
      auto sub =
          std::make_unique<sw::redis::Subscriber>(standalone->subscriber());
