...
MYSQL *Database::ConnectMYSQL(const std::string &host, int port) {
  ...
  if (!mysql_real_connect(c, host.c_str(), user_.c_str(), password_.c_str(),
...
bool Database::RunQuery(MYSQL *conn, const std::string &sql,
                        MYSQL_RES **out_res) {
...
static std::string escape(MYSQL *conn, const std::string &s) {
  ...
  unsigned long len =
      mysql_real_escape_string(conn, &out[0], s.c_str(), s.size());
  out.resize(len);
  return out;
}
...
Database::Database(const std::string &write_host, int write_port,
                   const std::string &read_hosts, int read_port,
                   const std::string &user, const std::string &password,
                   const std::string &db_name, int write_pool_size)
    : user_(user), password_(password), db_name_(db_name),
      write_host_(write_host), write_port_(write_port), read_port_(read_port),
  ...
  printf("[DB] write=%s:%d pool=%d reads=%zu slaves\n", write_host_.c_str(),
         write_port_, pool_size_, read_conns_.size());
}
...
bool Database::ExecWrite(const std::string &sql) {
  ...
  // 原子递增取索引, 无锁分发到不同连接
  size_t idx =
      write_idx_.fetch_add(1, std::memory_order_relaxed) % write_conns_.size();
...
MYSQL *Database::GetReadConn() {
  ...
  size_t idx =
      read_idx_.fetch_add(1, std::memory_order_relaxed) % read_conns_.size();
  return read_conns_[idx]->conn;
}

bool Database::ExecRead(const std::string &sql,
                        std::function<bool(MYSQL_RES *)> handler) {
  // 有从库: 走从库
  if (!read_conns_.empty()) {
    size_t idx =
        read_idx_.fetch_add(1, std::memory_order_relaxed) % read_conns_.size();
...
bool Database::Initialize() {
  ...
  exec("CREATE TABLE IF NOT EXISTS spreadsheets ("
       "id BIGINT AUTO_INCREMENT PRIMARY KEY, "
  ...
  mysql_query(
      c, "ALTER TABLE spreadsheets ADD COLUMN version INT NOT NULL DEFAULT 1");
  mysql_query(
      c, "ALTER TABLE users MODIFY COLUMN password_hash VARCHAR(256) NOT NULL");
  mysql_query(
      c, "ALTER TABLE users ADD COLUMN token_version INT NOT NULL DEFAULT 0");
  mysql_query(
      c, "ALTER TABLE files ADD COLUMN file_content LONGBLOB AFTER mime_type");
  ...
  // store
  mysql_query(
      c, "ALTER TABLE files ADD COLUMN storage_path VARCHAR(512) DEFAULT NULL");
  ...
  mysql_query(c, "ALTER TABLE spreadsheets ADD COLUMN idempotency_key CHAR(36) "
                 "UNIQUE NULL DEFAULT NULL");
  mysql_query(c, "ALTER TABLE files        ADD COLUMN idempotency_key CHAR(36) "
                 "UNIQUE NULL DEFAULT NULL");
  ...
  mysql_query(
      c,
      "ALTER TABLE spreadsheets ADD COLUMN user_id BIGINT NOT NULL DEFAULT 0");
  mysql_query(
      c,
      "ALTER TABLE files        ADD COLUMN user_id BIGINT NOT NULL DEFAULT 0");
  mysql_query(c, "ALTER TABLE spreadsheets ADD COLUMN storage_path "
                 "VARCHAR(512) DEFAULT NULL");
  // Backfill from users table (no-op if already populated).
  mysql_query(c, "UPDATE spreadsheets s JOIN users u ON s.username=u.username "
                 "SET s.user_id=u.id WHERE s.user_id=0");
  mysql_query(c, "UPDATE files f        JOIN users u ON f.username=u.username "
  ...
  mysql_query(c, "CREATE INDEX idx_sheets_user_time ON spreadsheets(user_id, "
                 "updated_at DESC)");
  ...
  mysql_query(
      c, "CREATE INDEX idx_files_user_time ON files(user_id, created_at DESC)");
  ...
  mysql_query(c, "ALTER TABLE users ADD UNIQUE INDEX idx_users_user_ver "
  ...
  mysql_query(
      c, "ALTER TABLE spreadsheets MODIFY COLUMN id BIGINT AUTO_INCREMENT");
  mysql_query(c, "ALTER TABLE spreadsheets MODIFY COLUMN user_id BIGINT NOT "
                 "NULL DEFAULT 0");
  mysql_query(c, "ALTER TABLE files MODIFY COLUMN id BIGINT AUTO_INCREMENT");
  mysql_query(
      c, "ALTER TABLE files MODIFY COLUMN user_id BIGINT NOT NULL DEFAULT 0");
  mysql_query(c, "ALTER TABLE undo_log MODIFY COLUMN id BIGINT AUTO_INCREMENT");
  mysql_query(c, "ALTER TABLE spreadsheets MODIFY COLUMN idempotency_key "
                 "CHAR(36) NULL DEFAULT NULL");
  mysql_query(c, "ALTER TABLE files MODIFY COLUMN idempotency_key CHAR(36) "
                 "NULL DEFAULT NULL");
...
bool Database::AddUser(const std::string &username,
  ...
  std::string sql = "INSERT INTO users (id, username, password_hash) VALUES (" +
                    std::to_string(snowflake_id) + "," + q(ec, username) +
                    ", " + q(ec, password_hash) + ")";
  return ExecWrite(sql);
}

bool Database::GetUser(const std::string &username,
  ...
  if (!wc->conn)
    return false;
  std::string sql =
      "SELECT password_hash FROM users WHERE username=" + q(wc->conn, username);
  MYSQL_RES *res = nullptr;
  if (!RunQuery(wc->conn, sql, &res))
    return false;
...
bool Database::UserExists(const std::string &username) {
  ...
  if (!wc->conn)
    return false;
  std::string sql =
      "SELECT 1 FROM users WHERE username=" + q(wc->conn, username);
  MYSQL_RES *res = nullptr;
  if (!RunQuery(wc->conn, sql, &res))
    return false;
...
int Database::GetTokenVersion(const std::string &username) {
  ...
  if (!wc->conn)
    return -1;
  std::string sql =
      "SELECT token_version FROM users WHERE username=" + q(wc->conn, username);
  MYSQL_RES *res = nullptr;
  if (!RunQuery(wc->conn, sql, &res))
    return -1;
  MYSQL_ROW row = mysql_fetch_row(res);
...
int64_t Database::GetUserId(const std::string &username) {
  ...
  if (!wc->conn)
    return -1;
  std::string sql =
      "SELECT id FROM users WHERE username=" + q(wc->conn, username);
  MYSQL_RES *res = nullptr;
  if (!RunQuery(wc->conn, sql, &res))
    return -1;
  MYSQL_ROW row = mysql_fetch_row(res);
...
static int countRows(const std::string &json) {
  ...
  for (size_t i = 0; i < json.size(); ++i)
    if (json[i] == ']' && i + 2 < json.size() && json[i + 1] == ',' &&
        json[i + 2] == '[')
      n++;
  return n;
}
...
bool Database::CreateSpreadsheet(int64_t user_id, const std::string &username,
  ...
  std::string cols = "id,user_id,username,name,description,headers_json,data_"
                     "json,row_count,col_count";
  std::string vals = std::to_string(snowflake_id) + "," +
                     std::to_string(user_id) + "," + q(ec, username) + "," +
                     q(ec, name) + "," + q(ec, desc) + "," +
                     q(ec, headers_json) + "," + q(ec, data_json) + "," +
...
bool Database::GetSpreadsheet(int64_t id, int64_t user_id,
                              SpreadsheetRow &out) {
  if (user_id <= 0)
    return false;
  std::string sql =
...
bool Database::ListSpreadsheets(int64_t user_id,
                                std::vector<SpreadsheetSummary> &out,
...
bool Database::UpdateSpreadsheet(int64_t id, const std::string &name,
  ...
  int rc = countRows(data_json), cc = countCols(headers_json);
  std::string sql =
      "UPDATE spreadsheets SET name=" + q(ec, name) +
      ",description=" + q(ec, desc) + ",headers_json=" + q(ec, headers_json) +
      ",data_json=" + q(ec, data_json) + ",row_count=" + std::to_string(rc) +
      ",col_count=" + std::to_string(cc) +
...
bool Database::GetSpreadsheetOwner(int64_t id, int64_t &owner_user_id,
                                   int *out_version) {
  // 强一致: 归属校验必须读主库
  if (write_conns_.empty())
    return false;
  ...
  if (!wc->conn)
    return false;
  std::string sql = "SELECT user_id, version FROM spreadsheets WHERE id=" +
                    std::to_string(id);
  MYSQL_RES *res = nullptr;
  if (!RunQuery(wc->conn, sql, &res))
    return false;
...
bool Database::CreateFile(int64_t user_id, const std::string &username,
                          const std::string &original_name, int64_t size,
                          const std::string &mime_type,
                          const std::string &storage_key, int64_t &out_id,
                          const std::string &idempotency_key) {
  MYSQL *ec = EscConn();
  int64_t snowflake_id = snowflake_ ? snowflake_->Next() : 0;
  ...
  std::string vals = std::to_string(snowflake_id) + "," +
                     std::to_string(user_id) + "," + q(ec, username) + "," +
                     q(ec, original_name) + "," + std::to_string(size) + "," +
                     q(ec, mime_type);
...
bool Database::UpdateFileContent(int64_t id, const std::string &content) {
  ...
  mysql_real_escape_string(wc->conn, esc, content.data(),
                           (unsigned long)content.size());
  std::string sql = "UPDATE files SET file_content='" + std::string(esc) +
                    "' WHERE id=" + std::to_string(id);
  free(esc);
...
bool Database::ListFiles(int64_t user_id, std::vector<FileRow> &out, int &total,
  ...
  if (page_size > 0) {
    ...
    sql = "SELECT id,username,original_name,size,mime_type,created_at "
          "FROM files " +
...
bool Database::GetFileStoragePath(int64_t id, std::string &storage_path) {
  ...
  storage_path.clear();
  std::string sql =
      "SELECT storage_path FROM files WHERE id=" + std::to_string(id);
  return ExecRead(sql, [&](MYSQL_RES *res) {
    MYSQL_ROW row = mysql_fetch_row(res);
    if (row && row[0])
      storage_path = row[0];
...
bool Database::GetSpreadsheetStoragePath(int64_t id,
                                         std::string &storage_path) {
  storage_path.clear();
  std::string sql =
      "SELECT storage_path FROM spreadsheets WHERE id=" + std::to_string(id);
  return ExecRead(sql, [&](MYSQL_RES *res) {
    MYSQL_ROW row = mysql_fetch_row(res);
    if (row && row[0])
      storage_path = row[0];
...
bool Database::UpdateSpreadsheetStoragePath(int64_t id,
                                            const std::string &storage_path) {
  auto *ec = EscConn();
  if (!ec)
    return false;
  std::string sql =
      "UPDATE spreadsheets SET storage_path=" + q(ec, storage_path) +
      " WHERE id=" + std::to_string(id);
  return ExecWrite(sql);
}
...
bool Database::WriteUndoLog(const std::string &xid,
                            const std::string &table_name, int64_t row_id,
                            const std::string &before_snapshot) {
  MYSQL *ec = EscConn();
  std::string sql =
      "INSERT INTO undo_log (xid,table_name,row_id,before_snapshot) VALUES (" +
      q(ec, xid) + "," + q(ec, table_name) + "," + std::to_string(row_id) +
      "," + q(ec, before_snapshot) + ")";
  return ExecWrite(sql);
}

bool Database::GetUndoLog(const std::string &xid, std::string &table_name,
  ...
  if (!wc->conn)
    return false;
  std::string sql = "SELECT table_name,row_id,before_snapshot FROM undo_log "
                    "WHERE xid=" +
...
int Database::PurgeOldUndoLogs(int days) {
  std::string sql =
      "DELETE FROM undo_log WHERE created_at < NOW() - INTERVAL " +
  ...
  while (health_running_) {
    ...
    for (size_t i = 0; i < read_conns_.size(); ++i) {
      ...
      if (rc->conn && mysql_ping(rc->conn) != 0) {
        ...
        mysql_close(rc->conn);
        rc->conn =
            ConnectMYSQL(read_hosts_[i % read_hosts_.size()], read_port_);
      }
    }
  }
}
...
ShardedDatabase::ShardedDatabase(
    int shard_count, const std::string &write_host_prefix, int write_port,
    const std::string &read_hosts_prefix, int read_port,
    const std::string &user, const std::string &password,
    const std::string &db_name_prefix, int write_pool_size)
    : shard_count_(shard_count), write_host_prefix_(write_host_prefix),
      read_hosts_prefix_(read_hosts_prefix), db_name_prefix_(db_name_prefix),
      write_port_(write_port), read_port_(read_port), user_(user),
      password_(password), write_pool_size_(write_pool_size) {
  for (int i = 0; i < shard_count; i++) {
    // shard_count==1: use host/db name as-is (no suffix), backward compat
    std::string wh = (shard_count == 1)
                         ? write_host_prefix
                         : write_host_prefix + "-" + std::to_string(i);
    std::string rh = (shard_count == 1)
                         ? read_hosts_prefix
                         : read_hosts_prefix + "-" + std::to_string(i);
    std::string db_name = (shard_count == 1)
                              ? db_name_prefix
                              : db_name_prefix + "_" + std::to_string(i);
    auto db = std::make_unique<Database>(wh, write_port, rh, read_port, user,
                                         password, db_name, write_pool_size);
    shards_.push_back(std::move(db));
  }
  printf("[ShardedDB] %d shards created (prefix: %s)\n", shard_count,
         write_host_prefix.c_str());
}
...
bool ShardedDatabase::AddUser(const std::string &username,
                              const std::string &password_hash) {
  return shards_[0]->AddUser(username, password_hash);
}
bool ShardedDatabase::GetUser(const std::string &username,
...
bool ShardedDatabase::CreateSpreadsheet(
    int64_t user_id, const std::string &username, const std::string &name,
    const std::string &desc, const std::string &headers_json,
    const std::string &data_json, int64_t &out_id,
    const std::string &idempotency_key) {
  return ShardFor(user_id)->CreateSpreadsheet(user_id, username, name, desc,
                                              headers_json, data_json, out_id,
                                              idempotency_key);
}
bool ShardedDatabase::GetSpreadsheet(int64_t id, int64_t user_id,
                                     SpreadsheetRow &out) {
  return ShardFor(user_id)->GetSpreadsheet(id, user_id, out);
}
bool ShardedDatabase::ListSpreadsheets(int64_t user_id,
                                       std::vector<SpreadsheetSummary> &out,
                                       int &total, int page, int page_size) {
  return ShardFor(user_id)->ListSpreadsheets(user_id, out, total, page,
                                             page_size);
}
bool ShardedDatabase::UpdateSpreadsheet(int64_t id, int64_t user_id,
                                        ...
                                        int version) {
  return ShardFor(user_id)->UpdateSpreadsheet(id, name, desc, headers_json,
                                              data_json, version);
}
...
// Broadcast: id is globally unique, try each shard
bool ShardedDatabase::GetSpreadsheetOwner(int64_t id, int64_t &owner_user_id,
                                          int *out_version) {
  for (auto &db : shards_)
...
bool ShardedDatabase::CreateFile(int64_t user_id, const std::string &username,
  ...
  return ShardFor(user_id)->CreateFile(user_id, username, original_name, size,
                                       mime_type, storage_key, out_id,
                                       idempotency_key);
}
bool ShardedDatabase::UpdateFileContent(int64_t id,
...
bool ShardedDatabase::GetFileStoragePath(int64_t id,
                                         std::string &storage_path) {
  for (auto &db : shards_)
...
// ---- Outbox ----
bool Database::InsertOutbox(const std::string &event_type,
                            const std::string &payload) {
  auto *ec = EscConn();
  if (!ec)
    return false;
  std::string sql = "INSERT INTO outbox (event_type, payload) VALUES (" +
                    q(ec, event_type) + "," + q(ec, payload) + ")";
  return ExecWrite(sql);
}
bool ShardedDatabase::InsertOutbox(int64_t user_id,
                                   const std::string &event_type,
                                   const std::string &payload) {
  return ShardFor(user_id)->InsertOutbox(event_type, payload);
}
bool ShardedDatabase::GetSpreadsheetStoragePath(int64_t id,
                                                std::string &storage_path) {
  for (auto &db : shards_)
...
bool ShardedDatabase::UpdateSpreadsheetStoragePath(
    int64_t id, const std::string &storage_path) {
  for (auto &db : shards_)
...
bool ShardedDatabase::WriteUndoLog(const std::string &xid,
...
bool ShardedDatabase::GetUndoLog(const std::string &xid,
                                 std::string &table_name, int64_t &row_id,
