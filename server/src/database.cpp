...
MYSQL* Database::ConnectMYSQL(const std::string& host, int port) {
    ...
    if (!c) { fprintf(stderr, "[DB] mysql_init failed\n"); return nullptr; }
...
// 转义辅助 — mysql_real_escape_string 是纯客户端函数, 同 charset 下任意连接结果一致
...
MYSQL* Database::EscConn() {
    for (auto& wc : write_conns_)
        if (wc->conn) return wc->conn;
    for (auto& rc : read_conns_)
        if (rc->conn) return rc->conn;
    return nullptr;
}
      ...
      pool_size_(write_pool_size)
{
    ...
    printf("[DB] write=%s:%d pool=%d reads=%zu slaves\n",
           write_host_.c_str(), write_port_, pool_size_, read_conns_.size());
}

Database::~Database() {
    StopHealthCheck();
    for (auto& wc : write_conns_) if (wc->conn) mysql_close(wc->conn);
    for (auto& rc : read_conns_) if (rc->conn) mysql_close(rc->conn);
}
...
bool Database::ExecWrite(const std::string& sql) {
    ...
    if (mysql_query(wc->conn, sql.c_str()) == 0) return true;
...
bool Database::Initialize() {
    if (write_conns_.empty() || !write_conns_[0]->conn) return false;
    ...
    // Object-storage migration: track where file content lives on disk/object store
    mysql_query(c, "ALTER TABLE files ADD COLUMN storage_path VARCHAR(512) DEFAULT NULL");

    // Idempotency key columns — NULL means "no deduplication" (multiple NULLs allowed by UNIQUE).
    // ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id) returns the existing id on replay.
    mysql_query(c, "ALTER TABLE spreadsheets ADD COLUMN idempotency_key CHAR(36) UNIQUE NULL DEFAULT NULL");
    mysql_query(c, "ALTER TABLE files        ADD COLUMN idempotency_key CHAR(36) UNIQUE NULL DEFAULT NULL");

    // user_id columns — integer FK to users.id; avoids username-as-FK design flaw.
    ...
    // Replace single-column indexes with composite covering indexes on user_id + sort column.
    // DROP first (idempotent — fails silently if already removed), then CREATE.
    ...
    mysql_query(c, "CREATE INDEX idx_sheets_user_time ON spreadsheets(user_id, updated_at DESC)");
    ...
    // undo_log: composite (xid, id DESC) eliminates the filesort on ORDER BY id DESC LIMIT 1.
    // Add created_at index to accelerate the periodic purge DELETE.
    ...
    mysql_query(c, "ALTER TABLE spreadsheets MODIFY COLUMN user_id BIGINT NOT NULL DEFAULT 0");
    ...
    mysql_query(c, "ALTER TABLE spreadsheets MODIFY COLUMN idempotency_key CHAR(36) NULL DEFAULT NULL");
    mysql_query(c, "ALTER TABLE files MODIFY COLUMN idempotency_key CHAR(36) NULL DEFAULT NULL");
...
bool Database::AddUser(const std::string& username, const std::string& password_hash) {
    ...
    std::string sql = "INSERT INTO users (id, username, password_hash) VALUES ("
                      + std::to_string(snowflake_id) + "," + q(ec, username) + ", " + q(ec, password_hash) + ")";
...
bool Database::GetUser(const std::string& username, std::string& password_hash_out) {
    ...
    MYSQL_RES* res = nullptr;
    if (!RunQuery(wc->conn, sql, &res)) return false;
    MYSQL_ROW row = mysql_fetch_row(res);
    bool ok = row != nullptr;
    if (ok) password_hash_out = row[0] ? row[0] : "";
...
int Database::GetTokenVersion(const std::string& username) {
    // 强一致: token 版本号必须读主库
    if (write_conns_.empty()) return -1;
    size_t idx = write_idx_.fetch_add(1, std::memory_order_relaxed) % write_conns_.size();
    auto& wc = write_conns_[idx];
    std::lock_guard<std::mutex> lock(wc->mtx);
    if (!wc->conn) return -1;
    ...
    MYSQL_RES* res = nullptr;
    if (!RunQuery(wc->conn, sql, &res)) return -1;
    MYSQL_ROW row = mysql_fetch_row(res);
...
int64_t Database::GetUserId(const std::string& username) {
    // Must read master to avoid replication lag causing stale id on fresh registration.
    if (write_conns_.empty()) return -1;
    size_t idx = write_idx_.fetch_add(1, std::memory_order_relaxed) % write_conns_.size();
    auto& wc = write_conns_[idx];
    std::lock_guard<std::mutex> lock(wc->mtx);
...
bool Database::ImportFromUsersJson(const std::string& json_path) {
    ...
    while (std::getline(f, line)) {
        ...
        for (char c : encoded) {
            ...
            valb += 6;
            if (valb >= 0) { decoded += static_cast<char>((val >> valb) & 0xFF); valb -= 8; }
        }
        if (AddUser(username, sha256::hash_password(decoded))) imported++;
    }
...
static int countRows(const std::string& json) {
    if (json.empty() || json == "[]") return 0;
    int n = 1;
    for (size_t i = 0; i < json.size(); ++i)
        if (json[i] == ']' && i + 2 < json.size() && json[i+1] == ',' && json[i+2] == '[') n++;
    return n;
}
...
bool Database::CreateSpreadsheet(int64_t user_id, const std::string& username,
    ...
    std::string cols = "id,user_id,username,name,description,headers_json,data_json,row_count,col_count";
    std::string vals = std::to_string(snowflake_id) + "," + std::to_string(user_id) + "," + q(ec, username) + "," + q(ec, name)
                       + "," + q(ec, desc) + "," + q(ec, headers_json) + "," + q(ec, data_json)
...
bool Database::ListSpreadsheets(int64_t user_id, std::vector<SpreadsheetSummary>& out,
    ...
    // Always obtain exact total via COUNT(*) — avoids loading all rows just to count them
    total = 0;
    ...
    if (page_size > 0) {
        ...
        // Delayed Join: subquery scans only (username, updated_at) covering index to find ids,
        // then join back to fetch full row data — avoids deep-page row reads.
        sql = "SELECT s.id,s.name,s.description,s.row_count,s.col_count,s.updated_at "
              "FROM spreadsheets s "
              "JOIN (SELECT id FROM spreadsheets " + where
    ...
    return ExecRead(sql, [&](MYSQL_RES* res) {
        ...
        while ((row = mysql_fetch_row(res))) {
            ...
            s.id = std::stoll(row[0]); s.name = row[1] ? row[1] : "";
...
bool Database::UpdateSpreadsheet(int64_t id, const std::string& name,
    ...
    MYSQL* ec = EscConn();
    int rc = countRows(data_json), cc = countCols(headers_json);
    std::string sql = "UPDATE spreadsheets SET name=" + q(ec, name)
        + ",description=" + q(ec, desc)
        + ",headers_json=" + q(ec, headers_json)
        + ",data_json=" + q(ec, data_json)
...
bool Database::GetSpreadsheetOwner(int64_t id, int64_t& owner_user_id, int* out_version) {
    ...
    if (row) {
        ...
        if (out_version) *out_version = row[1] ? std::stoi(row[1]) : 1;
    }
    mysql_free_result(res);
    return row != nullptr;
}
...
bool Database::GetFile(int64_t id, int64_t user_id, FileRow& out) {
    ...
    return ExecRead(sql, [&](MYSQL_RES* res) {
        MYSQL_ROW row = mysql_fetch_row(res);
        if (!row) return false;
        out.id = std::stoll(row[0]); out.username = row[1] ? row[1] : "";
        ...
        if (out.storage_path.empty() && row[6]) {
            ...
            if (lens) out.file_content.assign(row[6], lens[6]);
        }
        return true;
    });
}
...
bool Database::ListFiles(int64_t user_id, std::vector<FileRow>& out, int& total,
    ...
    // Always obtain exact total via COUNT(*) to avoid loading all rows just to count
    total = 0;
    ...
    if (page_size > 0) {
        int offset = page * page_size;
        // Delayed Join: subquery uses covering index (username, created_at) for id lookup,
        // then join back to read full row — avoids scanning large row data at deep offsets.
        sql = "SELECT f.id,f.username,f.original_name,f.size,f.mime_type,f.created_at "
              "FROM files f "
              "JOIN (SELECT id FROM files " + where
    ...
    return ExecRead(sql, [&](MYSQL_RES* res) {
        ...
        while ((row = mysql_fetch_row(res))) {
            FileRow f;
            f.id = std::stoll(row[0]); f.username = row[1] ? row[1] : "";
...
bool Database::GetFileOwner(int64_t id, int64_t& owner_user_id) {
    ...
    if (!RunQuery(wc->conn, sql, &res)) return false;
    MYSQL_ROW row = mysql_fetch_row(res);
    if (row) owner_user_id = row[0] ? std::stoll(row[0]) : 0;
    mysql_free_result(res);
    return row != nullptr;
}
...
bool Database::WriteUndoLog(const std::string& xid, const std::string& table_name,
    ...
    MYSQL* ec = EscConn();
    std::string sql = "INSERT INTO undo_log (xid,table_name,row_id,before_snapshot) VALUES ("
                      + q(ec, xid) + "," + q(ec, table_name) + ","
                      + std::to_string(row_id) + "," + q(ec, before_snapshot) + ")";
    return ExecWrite(sql);
}
...
bool Database::GetUndoLog(const std::string& xid, std::string& table_name,
    ...
    std::string sql = "SELECT table_name,row_id,before_snapshot FROM undo_log "
                      "WHERE xid=" + q(wc->conn, xid) + " ORDER BY id DESC LIMIT 1";
    MYSQL_RES* res = nullptr;
    if (!RunQuery(wc->conn, sql, &res)) return false;
    MYSQL_ROW row = mysql_fetch_row(res);
...
int Database::PurgeOldUndoLogs(int days) {
    std::string sql = "DELETE FROM undo_log WHERE created_at < NOW() - INTERVAL "
