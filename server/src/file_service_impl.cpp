...
static std::string FileListCacheKey(int64_t user_id, int64_t version, int page,
                                    int page_size) {
  std::string key =
      "u:" + std::to_string(user_id) + ":files:v" + std::to_string(version);
  if (page_size > 0)
...
grpc::Status FileServiceImpl::CreateFile(grpc::ServerContext *context,
  ...
  if (minio_ && minio_->IsConfigured() && !req->file_content().empty()) {
    ...
    if (!minio_->PutObject(storage_key, req->file_content(),
                           req->mime_type())) {
      resp->set_success(false);
  ...
  bool ok = db_->CreateFile(req->user_id(), username, req->original_name(),
                            req->size(), req->mime_type(), storage_key, id,
                            req->idempotency_key());
  ...
  // 发布 RabbitMQ 事件 + outbox 兜底
  {
    ...
    if (db_)
      db_->InsertOutbox(req->user_id(), "file.uploaded", ev.dump());
  }
  ...
  if (slog_)
    LOG_INFO(*slog_, "Created id=" + std::to_string(id) + " '" +
                         req->original_name() + "' by " + username +
...
grpc::Status FileServiceImpl::GetFile(grpc::ServerContext *context,
  ...
  int64_t owner_uid = 0;
  if (!db_ || !db_->GetFileOwner(req->id(), owner_uid) ||
  ...
  // 异步刷新函数
  auto async_refresh = [this, kNullMarker](int64_t id, int64_t uid,
                                           std::string ck, std::string tk,
                                           std::string lk) {
    FileRow row;
...
grpc::Status FileServiceImpl::DeleteFile(grpc::ServerContext *context,
...

  {
    ...
    if (db_)
      db_->InsertOutbox(req->user_id(), "file.deleted", ev.dump());
  }
