...
struct Client {
  ...
  bool PutObject(
    ...
    std::string auth = "AWS4-HMAC-SHA256 Credential=" + access_key + "/" +
  ...
  bool GetObject(const std::string &key, std::string &out_body) const {
    ...
    if (res->status != 200) {
      fprintf(stderr, "[MinIO] GetObject HTTP %d for key=%s: %s\n", res->status,
              key.c_str(), res->body.substr(0, 256).c_str());
      return false;
    }
  ...
  bool DeleteObject(const std::string &key) const {
    ...
    std::string canon_request = "DELETE\n" + uri + "\n\n" + canon_headers +
                                "\n" + signed_headers + "\n" + payload_hash;
  ...
  std::string PresignedGetUrl(const std::string &key,
                              int expires = 3600) const {
    std::string date8, dt16;
    get_datetime(date8, dt16);
    ...
    std::string query = "X-Amz-Algorithm=AWS4-HMAC-SHA256"
                        "&X-Amz-Credential=" +
                        url_encode(credential) + "&X-Amz-Date=" + dt16 +
