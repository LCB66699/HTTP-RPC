...
class CallLogger {
   public:
    ...
    void Log(const std::string &username, const std::string &service, const std::string &method, const json &params,
             const json &result, bool success, int64_t duration_us);
    std::vector<CallEntry> GetHistory(int limit = 50, int offset = 0, const std::string &service_filter = "",
