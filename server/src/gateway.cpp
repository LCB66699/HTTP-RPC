...
static std::shared_ptr<grpc::Channel> MakeChannel(const std::string &addr) {
    // Create operations are NOT retried at the gRPC level — idempotency is
    // handled by the client-supplied idempotency_key + server-side ON DUPLICATE
    // KEY instead. All other methods (reads + Update/Delete) may retry once on
    // UNAVAILABLE.
...
// Per-replica-aware gRPC call: retries once if the first attempt lands on a
// quarantined replica, giving gRPC round_robin a chance to pick a different
// subchannel. rpc_fn(ctx_ptr) → std::pair<grpc::Status, bool> where bool =
// rpc_ok && resp.success()
template <typename F>
static bool RepRetry(PerReplicaTracker &rep, const std::string &username, const std::string &raw_token, int timeout_sec,
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        ...
        out_peer = ctx.peer();
        if (ok) {
            rep.RecordSuccess(out_peer);
            return true;
        }
        rep.RecordFailure(out_peer);
        if (rep.AllowReplica(out_peer))
            return false;  // not quarantined, give up
    }
    return false;
}

Gateway::Gateway(const std::string &listen_addr, int listen_port, const std::string &grpc_auth_addr,
                 const std::string &grpc_sheet_addr, const std::string &grpc_file_addr, const std::string &jwt_secret,
                 const std::string &web_dir, const std::vector<std::string> &redis_cluster_seeds,
                 const std::string &redis_password, int redis_pool_size, int max_concurrent, int queue_timeout_ms,
                 const std::string &es_host)
    : listen_addr_(listen_addr),
      listen_port_(listen_port),
      grpc_auth_addr_(grpc_auth_addr),
      grpc_sheet_addr_(grpc_sheet_addr),
      grpc_file_addr_(grpc_file_addr),
      jwt_secret_(jwt_secret),
      web_dir_(web_dir),
      redis_cluster_seeds_(redis_cluster_seeds),
      redis_password_(redis_password),
      redis_pool_size_(redis_pool_size),
...
bool Gateway::VerifyAccessToken(const std::string &cookie_header, std::string &username, int64_t &user_id,
    ...
    if (JsonGet(payload, "type") != "access")
        return false;

    long exp = (long)JsonGetNum(payload, "exp");
    if (exp == 0 || exp < (long)std::time(nullptr))
        return false;
...
bool Gateway::VerifyAuth(const std::string &cookie_header, std::string &username, int64_t &user_id,
                         std::string &raw_token) const {
...
RpcResult Gateway::HandleLogin(const std::string &body, std::string &response, std::string &at_out,
                               std::string &rt_out) {
    std::string u = JsonGet(body, "username"), p = JsonGet(body, "password");
    if (u.empty() || p.empty()) {
        response = R"({"success":false,"error":"Username and password required"})";
        return RpcResult::BAD_REQUEST;
    }
    grpc::ClientContext ctx;
    ...
    if (ok) {
        at_out = resp.access_token();
        rt_out = resp.refresh_token();
        nlohmann::json r;
        r["success"] = true;
        r["username"] = u;
        r["_rt"] = rt_out;
...
// ---- 协程版 Login（token 由调用方通过 Set-Cookie 下发，此处仅返回
// username）----
Task<void> Gateway::HandleLoginCoro(std::string body, std::string &response) {
    ...
    req.set_username(u);
    req.set_password(p);
    auto r = co_await GrpcCallOf(auth_cq_.cq(), auth_stub_.get(), &rpc::AuthService::Stub::AsyncLogin, req, 5);
    bool ok = r.st.ok() && r.resp.success();
    // Coro path: token cannot be set as cookie here without access to the
    // response object. The h2c lambda must intercept the token; for now forward
    // it in body so the h2c lambda can set the cookie.
    response = ok ? "{\"success\":true,\"username\":" + JsonStr(u) + ",\"_tok\":" + JsonStr(r.resp.token()) + "}"
                  : "{\"success\":false,\"error\":" + JsonStr(r.resp.error()) + "}";
    co_return;
}

RpcResult Gateway::HandleRegister(const std::string &body, std::string &response, std::string &at_out,
                                  std::string &rt_out) {
    std::string u = JsonGet(body, "username"), p = JsonGet(body, "password");
    if (u.empty() || p.empty()) {
        response = R"({"success":false,"error":"Username and password required"})";
...
// Sheet handlers
RpcResult Gateway::HandleSheetCreate(const std::string &username, int64_t user_id, const std::string &body,
                                     const std::string &idempotency_key, const std::string &raw_token,
    ...
    if (ok) {
        nlohmann::json r;
        r["success"] = true;
        r["id"] = std::to_string(resp.id());
        response = r.dump();
        // 异步索引到 ES
        nlohmann::json idx_req;
        idx_req["id"] = resp.id();
        idx_req["user_id"] = user_id;
        idx_req["username"] = username;
        idx_req["name"] = name;
        idx_req["description"] = JsonGet(body, "description");
        idx_req["cell_content"] = JsonGet(body, "data_json");
        idx_req["row_count"] = (int)JsonGetNum(body, "row_count");
        idx_req["col_count"] = (int)JsonGetNum(body, "col_count");
        idx_req["type"] = "sheet";
        auto now = std::chrono::system_clock::now();
        auto now_t = std::chrono::system_clock::to_time_t(now);
        char buf[32];
        std::strftime(buf, sizeof(buf), "%Y-%m-%dT%H:%M:%SZ", std::gmtime(&now_t));
        idx_req["created_at"] = buf;
        idx_req["updated_at"] = buf;
        IndexToES("sheets_search", std::to_string(resp.id()), idx_req.dump());
    } else {
...
Task<void> Gateway::HandleSheetCreateCoro(std::string username, int64_t user_id, std::string body,
    ...
    req.set_data_json(JsonGet(body, "data_json"));
    req.set_idempotency_key(idempotency_key);
    auto r = co_await GrpcCallOf(sheet_cq_.cq(), sheet_stub_.get(),
                                 &rpc::SpreadsheetService::Stub::AsyncCreateSpreadsheet, req, 5);
    bool ok = r.st.ok() && r.resp.success();
    if (ok) {
        nlohmann::json j;
        j["success"] = true;
        j["id"] = std::to_string(r.resp.id());
        ;
        response = j.dump();
    } else {
        response = R"({"success":false,"error":"Failed"})";
...
// 协程版 Sheet Get
Task<void> Gateway::HandleSheetGetCoro(std::string username, int64_t user_id, std::string body, std::string &response) {
    int64_t id = JsonGetInt64(body, "id");
    rpc::GetSpreadsheetRequest req;
    req.set_id(id);
    req.set_user_id(user_id);
    auto r = co_await GrpcCallOf(sheet_cq_.cq(), sheet_stub_.get(), &rpc::SpreadsheetService::Stub::AsyncGetSpreadsheet,
                                 req, 5);
    if (!r.st.ok() || !r.resp.success()) {
        response = "{\"success\":false}";
        co_return;
    }
    const auto &s = r.resp.spreadsheet();
    response = "{\"success\":true,\"cache_source\":" + JsonStr(r.resp.cache_source()) + ",\"spreadsheet\":{\"id\":\"" +
               std::to_string(s.id()) + "\"" + ",\"name\":" + JsonStr(s.name()) +
               ",\"row_count\":" + std::to_string(s.row_count()) + ",\"updated_at\":" + JsonStr(s.updated_at()) + "}}";
    co_return;
}

// 协程版 Sheet List
Task<void> Gateway::HandleSheetListCoro(std::string username, int64_t user_id, int page, int page_size,
                                        std::string &response) {
    ...
    for (int i = 0; i < r.resp.sheets_size(); ++i) {
        ...
        arr += "{\"id\":\"" + std::to_string(s.id()) + "\",\"name\":" + JsonStr(s.name()) +
               ",\"description\":" + JsonStr(s.description()) + ",\"row_count\":" + std::to_string(s.row_count()) +
               ",\"col_count\":" + std::to_string(s.col_count()) + ",\"updated_at\":" + JsonStr(s.updated_at()) + "}";
    }
    arr += "]";
...
RpcResult Gateway::HandleSheetGet(const std::string &username, int64_t user_id, const std::string &body,
                                  const std::string &raw_token, PerReplicaTracker &rep, std::string &response) {
    int64_t id = JsonGetInt64(body, "id");
    rpc::GetSpreadsheetRequest req;
    ...
    if (!ok) {
        response = "{\"success\":false}";
        return RpcResult::TRANSPORT_FAILURE;
    }
    const auto &s = resp.spreadsheet();
    response = "{\"success\":true,\"cache_source\":" + JsonStr(resp.cache_source()) + ",\"spreadsheet\":{\"id\":\"" +
               std::to_string(s.id()) + "\"" + ",\"name\":" + JsonStr(s.name()) +
               ",\"description\":" + JsonStr(s.description()) + ",\"headers_json\":" + s.headers_json() +
               ",\"data_json\":" + s.data_json() + ",\"row_count\":" + std::to_string(s.row_count()) +
               ",\"col_count\":" + std::to_string(s.col_count()) + ",\"created_at\":" + JsonStr(s.created_at()) +
               ",\"updated_at\":" + JsonStr(s.updated_at()) + "}}";
    return RpcResult::SUCCESS;
}

RpcResult Gateway::HandleSheetList(const std::string &username, int64_t user_id, int page, int page_size,
                                   const std::string &raw_token, PerReplicaTracker &rep, std::string &response) {
    rpc::ListSpreadsheetsRequest req;
...
RpcResult Gateway::HandleSheetUpdate(const std::string &username, int64_t user_id, const std::string &body,
                                     const std::string &raw_token, PerReplicaTracker &rep, std::string &response) {
    int64_t id = JsonGetInt64(body, "id");
    ...
    req.set_id(id);
    req.set_user_id(user_id);
    req.set_name(JsonGet(body, "name"));
    req.set_description(JsonGet(body, "description"));
    req.set_headers_json(JsonGet(body, "headers_json"));
    req.set_data_json(JsonGet(body, "data_json"));
    std::string peer;
    ...
    if (ok) {
        ...
        idx_req["user_id"] = user_id;
        idx_req["username"] = username;
        idx_req["name"] = JsonGet(body, "name");
        idx_req["description"] = JsonGet(body, "description");
        idx_req["cell_content"] = JsonGet(body, "data_json");
        idx_req["row_count"] = (int)JsonGetNum(body, "row_count");
        idx_req["col_count"] = (int)JsonGetNum(body, "col_count");
        ...
        std::strftime(buf, sizeof(buf), "%Y-%m-%dT%H:%M:%SZ", std::gmtime(&now_t));
        idx_req["updated_at"] = buf;
        IndexToES("sheets_search", std::to_string(id), idx_req.dump());
    }
    return ok ? RpcResult::SUCCESS : RpcResult::TRANSPORT_FAILURE;
}

RpcResult Gateway::HandleSheetDelete(const std::string &username, int64_t user_id, const std::string &body,
                                     const std::string &raw_token, PerReplicaTracker &rep, std::string &response) {
    int64_t id = JsonGetInt64(body, "id");
...
// File handlers
RpcResult Gateway::HandleFileList(const std::string &username, int64_t user_id, int page, int page_size,
                                  const std::string &raw_token, PerReplicaTracker &rep, std::string &response) {
    rpc::ListFilesRequest req;
...
RpcResult Gateway::HandleFileDelete(const std::string &username, int64_t user_id, const std::string &body,
                                    const std::string &raw_token, PerReplicaTracker &rep, std::string &response) {
    int64_t id = JsonGetInt64(body, "id");
    rpc::DeleteFileRequest req;
...
RpcResult Gateway::HandleTxBegin(const std::string &, const std::string &body, std::string &response) {
    if (!tx_manager_) {
        response = "{\"error\":\"TM disabled\"}";
        return RpcResult::BUSINESS_FAILURE;
    }
...
bool Gateway::WaitForBackends(int timeout_sec) {
    ...
    if (auth_ch_ && !auth_ch_->WaitForConnected(deadline)) {
        fprintf(stderr, "[Gateway] auth backend timeout\n");
        return false;
    }
    if (sheet_ch_ && !sheet_ch_->WaitForConnected(deadline)) {
        fprintf(stderr, "[Gateway] sheet backend timeout\n");
        return false;
    }
    if (file_ch_ && !file_ch_->WaitForConnected(deadline)) {
        fprintf(stderr, "[Gateway] file backend timeout\n");
        return false;
    }
...
RpcResult Gateway::HandleHealth(std::string &response) {
    ...
    if (redis_ && redis_->IsConnected()) {
        ...
        static const char *kKnownNodes[] = {"auth-50051",        "auth-50052",        "spreadsheet-50051",
                                            "spreadsheet-50052", "spreadsheet-50053", "file-50051",
                                            "file-50052",        "gateway-8080",      "gateway-8081"};
...
std::string Gateway::HandleMetrics() const {
    ...
    out +=
        "# HELP gateway_requests_errors_total Total HTTP request errors "
        "(4xx/5xx).\n";
...
std::string Gateway::HandleSearch(const std::string &username, int64_t user_id, const std::string &body) const {
    if (es_host_.empty())
        return R"({"error":"Search service not configured"})";

    std::string q = JsonGet(body, "q");
    std::string scope_str = JsonGet(body, "scope");  // e.g. "sheets,files"
    std::string sort_by = JsonGet(body, "sort");     // "relevance"|"date_desc"|"date_asc"
    int page = (int)JsonGetNum(body, "page");
    int page_size = (int)JsonGetNum(body, "page_size");
    if (page <= 0)
        page = 1;
    ...
    if (q.empty())
        return R"({"error":"Query 'q' is required"})";

    // 确定搜索的索引
    bool search_sheets = true, search_files = true;
    if (!scope_str.empty()) {
        search_sheets = (scope_str.find("sheets") != std::string::npos);
        search_files = (scope_str.find("files") != std::string::npos);
    }
    std::string indices;
    if (search_sheets && search_files)
        indices = "sheets_search,files_search";
    else if (search_sheets)
    ...
    multi_match["multi_match"]["query"] = q;
    if (search_sheets && search_files) {
        multi_match["multi_match"]["fields"] =
            nlohmann::json::array({"name^3", "description^2", "cell_content", "original_name^3", "content_text"});
    } else if (search_sheets) {
        multi_match["multi_match"]["fields"] = nlohmann::json::array({"name^3", "description^2", "cell_content"});
    } else {
        multi_match["multi_match"]["fields"] = nlohmann::json::array({"original_name^3", "content_text"});
    }
    multi_match["multi_match"]["type"] = "best_fields";
    multi_match["multi_match"]["fuzziness"] = "AUTO";
    query["bool"]["must"].push_back(multi_match);

    // 权限过滤 (非 admin 只查自己的数据)
    bool is_admin = IsAdminUser(username);
    if (!is_admin) {
        nlohmann::json term_filter;
        term_filter["term"]["user_id"] = user_id;
        query["bool"]["filter"] = nlohmann::json::array({term_filter});
    }
    ...
    if (sort_by == "date_desc") {
        es_query["sort"] =
            nlohmann::json::array({{{"updated_at", "desc"}}, {{"created_at", "desc"}}, {{"_score", "desc"}}});
    } else if (sort_by == "date_asc") {
        es_query["sort"] =
            nlohmann::json::array({{{"updated_at", "asc"}}, {{"created_at", "asc"}}, {{"_score", "desc"}}});
    }

    // 高亮
    es_query["highlight"]["fields"]["*"] = {{"fragment_size", 80}, {"number_of_fragments", 3}};
    es_query["highlight"]["pre_tags"] = nlohmann::json::array({"<em>"});
    es_query["highlight"]["post_tags"] = nlohmann::json::array({"</em>"});

    // 调用 ES REST API
    httplib::Client es_cli(es_host_);
    es_cli.set_connection_timeout(3, 0);
    es_cli.set_read_timeout(5, 0);
    std::string req_body = es_query.dump();
    auto es_res = es_cli.Post("/" + indices + "/_search", req_body, "application/json");
    if (!es_res || es_res->status != 200) {
        fprintf(stderr, "[Search] ES error: %s\n", es_res ? es_res->body.c_str() : "connection failed");
        return R"({"error":"Search service temporarily unavailable"})";
    }
    ...
    try {
        ...
        int64_t total =
            sr.value("hits", nlohmann::json::object()).value("total", nlohmann::json::object()).value("value", 0);
        ...
        for (auto &hit : sr["hits"]["hits"]) {
            ...
            item["id"] = std::to_string(src.value("id", 0LL));
            item["score"] = hit.value("_score", 0.0);

            if (src.value("type", "") == "sheet") {
                item["name"] = src.value("name", "");
                item["description"] = src.value("description", "");
                item["username"] = src.value("username", "");
                item["row_count"] = src.value("row_count", 0);
                item["col_count"] = src.value("col_count", 0);
                item["updated_at"] = src.value("updated_at", "");
            } else {
                item["original_name"] = src.value("original_name", "");
                item["mime_type"] = src.value("mime_type", "");
                item["size"] = src.value("size", 0);
                item["username"] = src.value("username", "");
                item["created_at"] = src.value("created_at", "");
            }
        ...
        return R"({"error":"Search parse error"})";
    }
}

// 直接索引文档到 ES (fire-and-forget, 绕过 doc-parser)
void Gateway::IndexToES(const std::string &index_name, const std::string &doc_id, const std::string &json_body) const {
    if (es_host_.empty())
        return;
    std::thread([host = es_host_, idx = index_name, id = doc_id, body = json_body]() {
        httplib::Client cli(host);
        cli.set_connection_timeout(2, 0);
        cli.set_read_timeout(5, 0);
        std::string path = "/" + idx + "/_doc/" + id;
        auto res = cli.Put(path, body, "application/json");
        if (!res || (res->status != 200 && res->status != 201)) {
            fprintf(stderr, "[ES] index %s/%s failed: %s\n", idx.c_str(), id.c_str(),
                    res ? res->body.c_str() : "no response");
        }
    }).detach();
}

// 异步通知 doc-parser 服务 (fire-and-forget, 不影响主流程)
void Gateway::NotifyDocParser(const std::string &endpoint, const std::string &json_body) const {
    if (doc_parser_host_.empty())
        return;
    std::thread([host = doc_parser_host_, ep = endpoint, body = json_body]() {
        httplib::Client cli(host);
        cli.set_connection_timeout(1, 0);
        cli.set_read_timeout(30, 0);
        auto res = cli.Post(ep, body, "application/json");
        if (!res || res->status != 200) {
            fprintf(stderr, "[doc-parser] notify %s failed: %s\n", ep.c_str(), res ? res->body.c_str() : "no response");
        }
    }).detach();
}
...
bool Gateway::HandleSystemStatus(std::string &response) {
    ...
    response = std::string("{") + "\"services\":{" + "\"auth\":{\"channel\":\"" +
               state_str(auth_ch_ ? auth_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN) + "\",\"breaker\":\"" +
               cb_auth_.StateStr() + "\"}," + "\"sheet\":{\"channel\":\"" +
               state_str(sheet_ch_ ? sheet_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN) + "\",\"breaker\":\"" +
               cb_sheet_.StateStr() + "\"}," + "\"file\":{\"channel\":\"" +
               state_str(file_ch_ ? file_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN) + "\",\"breaker\":\"" +
               cb_file_.StateStr() + "\"}}," + "\"errors\":{" + "\"auth\":" + std::to_string(err_auth) + "," +
               "\"spreadsheet\":" + std::to_string(err_spreadsheet) + "," + "\"file\":" + std::to_string(err_file) +
               "}," + "\"log_level\":\"active\"" + "}";
    return true;
}
...
bool Gateway::Start() {
    ...
    // require_auth_full: extracts both username (for logging) and user_id (for DB
    // queries)
    auto require_auth_full = [this](const auto &req, auto &res, std::string &u, int64_t &uid,
                                    std::string &tok) -> bool {
    ...
    auto require_auth = [&require_auth_full](const auto &req, auto &res, std::string &u, std::string &tok) -> bool {
        int64_t uid = 0;
        return require_auth_full(req, res, u, uid, tok);
    };
    auto require_admin = [this](auto &req, auto &res) -> bool {
        std::string role = GetRoleFromCookie(req.get_header_value("Cookie"));
        if (role != "admin") {
            res.status = 403;
            res.set_content(R"({"error":"admin required"})", "application/json");
            return false;
        }
        return true;
    };

    // Cookie属性: HttpOnly防JS读取; SameSite=Strict防CSRF; Path=/限制范围;
    // 24h过期
    static const std::string kSetCookieFmt = "rpc_at=%s; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=900";
    static const std::string kClearCookie = "rpc_at=; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=0";
    ...
    auto login = [this, &kSetCookieFmt](auto &req, auto &res) {
        ...
        if (redis_ && redis_->IsConnected() && !login_user.empty()) {
            if (redis_->GetInt("rate:block:" + login_user) >= 1) {
                res.status = 429;
                res.set_content(
                    "{\"success\":false,\"error\":\"Account temporarily "
                    "locked. Retry later.\"}",
                    "application/json");
                return;
            }
            if (redis_->GetInt("rate:login:" + login_user + ":total") >= 15) {
                ...
                res.status = 429;
                res.set_content(
                    "{\"success\":false,\"error\":\"Account locked due to "
                    "too many attempts.\"}",
                    "application/json");
                return;
            }
        ...
        switch (result) {
            case RpcResult::SUCCESS:
                cb_auth_.RecordResult(true, elapsed);
                break;
            case RpcResult::TRANSPORT_FAILURE:
                cb_auth_.RecordResult(false, elapsed);
                break;
            case RpcResult::BUSINESS_FAILURE:
                break;
        ...
        if (!at.empty()) {
            char buf[2048];
            snprintf(buf, sizeof(buf), kSetCookieFmt.c_str(), at.c_str());
    ...
    auto refresh_tok = [this](auto &req, auto &res) {
        ...
        rpc::RefreshTokenRequest rreq;
        rpc::RefreshTokenResponse rresp;
        rreq.set_username(u);
        rreq.set_refresh_token(rt);
        grpc::ClientContext ctx;
        ctx.set_deadline(std::chrono::system_clock::now() + std::chrono::seconds(2));
        auto st = auth_stub_->RefreshToken(&ctx, rreq, &rresp);
        if (!st.ok() || !rresp.success()) {
            res.status = 401;
            res.set_content(R"({"error":"Invalid refresh token"})", "application/json");
            return;
        }
        char buf[2048];
        snprintf(buf, sizeof(buf), kSetCookieFmt.c_str(), rresp.access_token().c_str());
        res.set_header("Set-Cookie", buf);
        res.set_content(R"({"success":true})", "application/json");
    };
    auto services = [&require_auth, this](auto &req, auto &res) {
        std::string u, tok;
        if (!require_auth(req, res, u, tok))
            return;
        std::string r;
        HandleServices(r);
        res.set_content(r, "application/json");
    ...
    auto health = [this](auto &req, auto &res) {
        std::string role = GetRoleFromCookie(req.get_header_value("Cookie"));
        if (role != "admin") {
            res.set_content(R"({"gateway":"READY"})", "application/json");
            return;
        }
        std::string r;
        HandleHealth(r);
    ...
    auto sys_status = [&require_auth, this](auto &req, auto &res) {
        ...
        std::string r;
        HandleSystemStatus(r);
        res.set_content(r, "application/json");
    };
    auto tx_begin = [this, &require_auth](auto &req, auto &res) {
        std::string u, tok;
        if (!require_auth(req, res, u, tok))
            return;
        std::string r;
        HandleTxBegin(u, req.body, r);
        res.set_content(r, "application/json");
    };
    ...
    auto file_up = [this, &require_auth_full](auto &req, auto &res) {
        std::string u, tok;
        int64_t uid = 0;
        if (!require_auth_full(req, res, u, uid, tok))
            return;
        ...
        if (file.filename.empty()) {
            res.set_content("{\"success\":false,\"error\":\"No file\"}", "application/json");
            res.status = 400;
            return;
        }
        ...
        rpc::CreateFileRequest freq;
        rpc::CreateFileResponse fresp;
        freq.set_user_id(uid);
        freq.set_original_name(file.filename);
        freq.set_size(file.content.size());
        freq.set_mime_type(mime);
        ...
        for (int attempt = 0; attempt < 2; ++attempt) {
            ...
            if (!st.ok()) {
                ...
                if (rep_file_.AllowReplica(peer))
                    break;  // not quarantined, give up
            } else
                break;  // business failure, no retry
        }
        auto elapsed =
            std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t0).count();
        if (!st.ok() || !fresp.success()) {
            if (!st.ok())
                cb_file_.RecordResult(false, elapsed);
            res.set_content("{\"success\":false,\"error\":\"Upload failed\"}", "application/json");
            return;
        }
        cb_file_.RecordResult(true, elapsed);
        rep_file_.RecordSuccess(peer);
        res.set_content("{\"success\":true,\"id\":\"" + std::to_string(fresp.id()) + "\"}", "application/json");

        // 直接索引文件元数据到 ES (文件名可搜索, 文件内容等 doc-parser 解析)
        nlohmann::json file_idx;
        auto now = std::chrono::system_clock::now();
        auto now_t = std::chrono::system_clock::to_time_t(now);
        char time_buf[32];
        std::strftime(time_buf, sizeof(time_buf), "%Y-%m-%dT%H:%M:%SZ", std::gmtime(&now_t));
        file_idx["id"] = fresp.id();
        file_idx["user_id"] = uid;
        file_idx["username"] = u;
        file_idx["original_name"] = file.filename;
        file_idx["mime_type"] = mime;
        file_idx["size"] = (int64_t)file.content.size();
        file_idx["content_text"] = "";
        file_idx["created_at"] = time_buf;
        file_idx["type"] = "file";
        IndexToES("files_search", std::to_string(fresp.id()), file_idx.dump());
    };
    ...
    auto file_down = [this, &require_auth_full](auto &req, auto &res) {
        ...
        if (!cb_file_.AllowRequest()) {
            res.set_content("{\"error\":\"circuit open\"}", "application/json");
            return;
        }
        rpc::GetFileRequest freq;
        rpc::GetFileResponse fresp;
        freq.set_id(std::stoll(id_str));
        freq.set_user_id(uid);
        auto t0 = std::chrono::steady_clock::now();
        grpc::Status st;
        std::string peer;
        ...
        cb_file_.RecordResult(true, elapsed);
        rep_file_.RecordSuccess(peer);
        // Body comes from MySQL BLOB or MinIO via File service (no browser redirect
        // to :443/:9000).
        ...
        if (dl_dot != std::string::npos) {
            ...
            if (dl_ext == ".png")
                dl_mime = "image/png";
            else if (dl_ext == ".jpg" || dl_ext == ".jpeg")
                dl_mime = "image/jpeg";
            else if (dl_ext == ".gif")
                dl_mime = "image/gif";
            else if (dl_ext == ".pdf")
    ...
    auto with_cb = [this](CircuitBreaker &cb, auto inner) {
        return [this, &cb, inner = std::move(inner)](auto &req, auto &res) {
            ...
            if (!cb.AllowRequest()) {
                res.status = 503;
                char buf[64];
                snprintf(buf, sizeof(buf), "%d", cb.TimeoutSec());
            ...
            // ② 并发控制 → 排队 → 超时 503
            auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(queue_timeout_ms_);
    ...
    auto sh_get = [this](auto &req, std::string &r) {
        std::string u;
        int64_t uid = 0;
        std::string tok;
        if (!VerifyAuth(req.get_header_value("Cookie"), u, uid, tok)) {
        ...
        return HandleSheetGet(u, uid, req.body, tok, rep_sheet_, r);
    };
    auto sh_update = [this](auto &req, std::string &r) {
        std::string u;
        int64_t uid = 0;
        std::string tok;
        if (!VerifyAuth(req.get_header_value("Cookie"), u, uid, tok)) {
        ...
        return HandleSheetUpdate(u, uid, req.body, tok, rep_sheet_, r);
    };
    auto sh_delete = [this](auto &req, std::string &r) {
        std::string u;
        int64_t uid = 0;
        std::string tok;
        if (!VerifyAuth(req.get_header_value("Cookie"), u, uid, tok)) {
        ...
        return HandleSheetDelete(u, uid, req.body, tok, rep_sheet_, r);
    };
    ...
    auto fl_delete = [this](auto &req, std::string &r) {
        std::string u;
        int64_t uid = 0;
        std::string tok;
        if (!VerifyAuth(req.get_header_value("Cookie"), u, uid, tok)) {
        ...
        return HandleFileDelete(u, uid, req.body, tok, rep_file_, r);
    };
    ...
    svr.Get("/metrics", [this](auto &, auto &res) { res.set_content(HandleMetrics(), "text/plain; version=0.0.4"); });
    ...
    auto search_handler = [this, &require_auth_full](auto &req, auto &res) {
        std::string u, tok;
        int64_t uid = 0;
        if (!require_auth_full(req, res, u, uid, tok))
            return;
        std::string r = HandleSearch(u, uid, req.body);
        res.set_content(r, "application/json");
    };
    svr.Post("/api/search", search_handler);
    ...
    http_svr->Get("/metrics",
                  [this](auto &, auto &res) { res.set_content(HandleMetrics(), "text/plain; version=0.0.4"); });
    ...
    http_svr->Post("/api/search", [this, &require_auth_full](auto &req, auto &res) {
        std::string u, tok;
        int64_t uid = 0;
        if (!require_auth_full(req, res, u, uid, tok))
            return;
    ...
    if (!redis_cluster_seeds_.empty()) {
        ...
        if (redis_->Connect()) {
            ...
            printf(
                "[Gateway] Circuit breakers connected to Redis (shared state "
                "enabled)\n");
        }
    }
