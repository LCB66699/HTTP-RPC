...
static bool RepRetry(PerReplicaTracker &rep, const std::string &username,
                     const std::string &raw_token, int timeout_sec, F &&rpc_fn,
                     std::string &out_peer) {
...
Gateway::Gateway(const std::string &listen_addr, int listen_port,
                 ...
                 const std::string &jwt_secret, const std::string &web_dir,
                 const std::vector<std::string> &redis_cluster_seeds,
                 const std::string &redis_password, int redis_pool_size,
                 int max_concurrent, int queue_timeout_ms,
                 const std::string &es_host)
    : listen_addr_(listen_addr), listen_port_(listen_port),
      grpc_auth_addr_(grpc_auth_addr), grpc_sheet_addr_(grpc_sheet_addr),
      grpc_file_addr_(grpc_file_addr), jwt_secret_(jwt_secret),
      web_dir_(web_dir), redis_cluster_seeds_(redis_cluster_seeds),
...
std::string Gateway::JsonGet(const std::string &json_str,
...
double Gateway::JsonGetNum(const std::string &json_str,
                           const std::string &key) {
  try {
    auto j = nlohmann::json::parse(json_str);
    auto it = j.find(key);
...
int64_t Gateway::JsonGetInt64(const std::string &json_str,
                              const std::string &key) {
  try {
    auto j = nlohmann::json::parse(json_str);
    auto it = j.find(key);
...
std::string Gateway::CreateJWT(const std::string &username) const {
  ...
  return jwt::create("{\"username\":" + JsonStr(username) +
...
std::string Gateway::CreateAccessToken(const std::string &username,
                                       int64_t uid) const {
...
std::string Gateway::CreateRefreshToken(const std::string &username,
                                        int64_t uid) const {
...
bool Gateway::VerifyAccessToken(const std::string &cookie_header,
  ...
  std::string token = cookie_header.substr(
...
bool Gateway::VerifyAuth(const std::string &cookie_header,
                         std::string &username, int64_t &user_id,
                         std::string &raw_token) const {
...
Task<void> Gateway::HandleLoginCoro(std::string body, std::string &response) {
  ...
  req.set_username(u);
  req.set_password(p);
  auto r = co_await GrpcCallOf(auth_cq_.cq(), auth_stub_.get(),
  ...
  response =
      ok ? "{\"success\":true,\"username\":" + JsonStr(u) +
               ",\"_tok\":" + JsonStr(r.resp.token()) + "}"
         : "{\"success\":false,\"error\":" + JsonStr(r.resp.error()) + "}";
  co_return;
}

RpcResult Gateway::HandleRegister(const std::string &body,
                                  std::string &response, std::string &at_out,
                                  std::string &rt_out) {
  std::string u = JsonGet(body, "username"), p = JsonGet(body, "password");
  if (u.empty() || p.empty()) {
    response = R"({"success":false,"error":"Username and password required"})";
...
// Sheet handlers
RpcResult Gateway::HandleSheetCreate(const std::string &username,
                                     int64_t user_id, const std::string &body,
                                     const std::string &idempotency_key,
                                     const std::string &raw_token,
                                     PerReplicaTracker &rep,
                                     std::string &response) {
...
// 协程版 Sheet Create
Task<void> Gateway::HandleSheetCreateCoro(std::string username, int64_t user_id,
                                          std::string body,
...
Task<void> Gateway::HandleSheetGetCoro(std::string username, int64_t user_id,
  ...
  response =
      "{\"success\":true,\"cache_source\":" + JsonStr(r.resp.cache_source()) +
...
RpcResult Gateway::HandleSheetGet(const std::string &username, int64_t user_id,
  ...
  response =
      "{\"success\":true,\"cache_source\":" + JsonStr(resp.cache_source()) +
      ",\"spreadsheet\":{\"id\":\"" + std::to_string(s.id()) + "\"" +
      ",\"name\":" + JsonStr(s.name()) +
      ",\"description\":" + JsonStr(s.description()) +
...
RpcResult Gateway::HandleSheetUpdate(const std::string &username,
                                     int64_t user_id, const std::string &body,
                                     const std::string &raw_token,
                                     PerReplicaTracker &rep,
                                     std::string &response) {
...
RpcResult Gateway::HandleSheetDelete(const std::string &username,
                                     int64_t user_id, const std::string &body,
                                     const std::string &raw_token,
                                     PerReplicaTracker &rep,
                                     std::string &response) {
...
RpcResult Gateway::HandleFileList(const std::string &username, int64_t user_id,
  ...
  for (int i = 0; i < resp.files_size(); ++i) {
    ...
    arr += "{\"id\":\"" + std::to_string(f.id()) +
           "\",\"original_name\":" + JsonStr(f.original_name()) +
           ",\"size\":" + std::to_string(f.size()) +
           ",\"mime_type\":" + JsonStr(f.mime_type()) +
           ",\"created_at\":" + JsonStr(f.created_at()) + "}";
  }
  arr += "]";
  response = "{\"success\":true,\"total\":" + std::to_string(resp.total()) +
             ",\"files\":" + arr + "}";
  return RpcResult::SUCCESS;
}

RpcResult Gateway::HandleFileDelete(const std::string &username,
                                    int64_t user_id, const std::string &body,
                                    const std::string &raw_token,
                                    PerReplicaTracker &rep,
                                    std::string &response) {
...
RpcResult Gateway::HandleTxBegin(const std::string &, const std::string &body,
                                 std::string &response) {
  if (!tx_manager_) {
...
RpcResult Gateway::HandleHealth(std::string &response) {
  ...
  result["channels"]["auth"]["state"] =
      state_str(auth_ch_ ? auth_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN);
  result["channels"]["auth"]["breaker"] = cb_auth_.StateStr();
  result["channels"]["sheet"]["state"] =
      state_str(sheet_ch_ ? sheet_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN);
  result["channels"]["sheet"]["breaker"] = cb_sheet_.StateStr();
  result["channels"]["file"]["state"] =
      state_str(file_ch_ ? file_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN);
  ...
  if (redis_ && redis_->IsConnected()) {
    ...
    static const char *kKnownNodes[] = {
        "auth-50051",        "auth-50052",        "spreadsheet-50051",
...
RpcResult Gateway::HandleHistory(const std::string &username,
                                 std::string &response) {
  if (!redis_ || !redis_->IsConnected()) {
  ...
  response = "{\"history\":" + arr +
             ",\"total\":" + std::to_string(redis_->GetCallCount(username)) +
             "}";
  return RpcResult::SUCCESS;
}
...
std::string Gateway::HandleMetrics() const {
  ...
  out += "# HELP gateway_requests_errors_total Total HTTP request errors "
         "(4xx/5xx).\n";
  out += "# TYPE gateway_requests_errors_total counter\n";
  out += "gateway_requests_errors_total " + std::to_string(errors) + "\n";
  out += "# HELP circuit_breaker_state Circuit breaker state "
         "(0=CLOSED,1=OPEN,2=HALF_OPEN).\n";
  out += "# TYPE circuit_breaker_state gauge\n";
  out += "circuit_breaker_state{name=\"auth\"} " +
         std::to_string(cb_state(cb_auth_)) + "\n";
  out += "circuit_breaker_state{name=\"sheet\"} " +
         std::to_string(cb_state(cb_sheet_)) + "\n";
  out += "circuit_breaker_state{name=\"file\"} " +
         std::to_string(cb_state(cb_file_)) + "\n";
  return out;
}

std::string Gateway::HandleSearch(const std::string &username, int64_t user_id,
  ...
  std::string sort_by =
      JsonGet(body, "sort"); // "relevance"|"date_desc"|"date_asc"
  ...
  if (search_sheets && search_files) {
    multi_match["multi_match"]["fields"] =
        nlohmann::json::array({"name^3", "description^2", "cell_content",
                               "original_name^3", "content_text"});
  } else if (search_sheets) {
    multi_match["multi_match"]["fields"] =
        nlohmann::json::array({"name^3", "description^2", "cell_content"});
  } else {
    multi_match["multi_match"]["fields"] =
        nlohmann::json::array({"original_name^3", "content_text"});
  }
  ...
  if (sort_by == "date_desc") {
    es_query["sort"] = nlohmann::json::array({{{"updated_at", "desc"}},
                                              {{"created_at", "desc"}},
                                              {{"_score", "desc"}}});
  } else if (sort_by == "date_asc") {
    es_query["sort"] = nlohmann::json::array({{{"updated_at", "asc"}},
                                              {{"created_at", "asc"}},
                                              {{"_score", "desc"}}});
  }

  // 高亮
  es_query["highlight"]["fields"]["*"] = {{"fragment_size", 80},
  ...
  auto es_res =
      es_cli.Post("/" + indices + "/_search", req_body, "application/json");
  ...
  try {
    ...
    int64_t total = sr.value("hits", nlohmann::json::object())
                        .value("total", nlohmann::json::object())
                        .value("value", 0);
...
void Gateway::IndexToES(const std::string &index_name,
                        const std::string &doc_id,
                        const std::string &json_body) const {
  if (es_host_.empty())
    return;
  std::thread([host = es_host_, idx = index_name, id = doc_id,
               body = json_body]() {
...
void Gateway::NotifyDocParser(const std::string &endpoint,
                              const std::string &json_body) const {
...
bool Gateway::HandleSystemStatus(std::string &response) {
  ...
  response =
      std::string("{") + "\"services\":{" + "\"auth\":{\"channel\":\"" +
      state_str(auth_ch_ ? auth_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN) +
      ...
      "\"file\":{\"channel\":\"" +
      state_str(file_ch_ ? file_ch_->GetState(true) : GRPC_CHANNEL_SHUTDOWN) +
      "\",\"breaker\":\"" + cb_file_.StateStr() + "\"}}," + "\"errors\":{" +
...
RpcResult Gateway::HandleBreakerStats(std::string &response) {
  ...
  for (auto *cb : {&cb_auth_, &cb_sheet_, &cb_file_}) {
    ...
    const char *name = (cb == &cb_auth_)    ? "auth"
                       : (cb == &cb_sheet_) ? "sheet"
                                            : "file";
    r[name] = s;
  }
...
bool Gateway::HandleStressRun(std::string &response, const std::string &token) {
  ...
  auto extract_pct = [&](const std::string &pct) -> std::string {
    ...
    while (val_start < output.size() &&
           (output[val_start] == ' ' || output[val_start] == '\t'))
      val_start++;
    auto end = output.find('\n', val_start);
...
bool Gateway::Start() {
  ...
  // ---- h2c server (8080) ----
  Http2Server svr;
  svr.set_pre_routing_handler([](const Http2Server::Request &,
                                 Http2Server::Response &res) {
  ...
  svr.Options(R"(/api/.*)",
              [](const Http2Server::Request &, Http2Server::Response &res) {
                res.status = 204;
              });
  ...
  http_svr->set_pre_routing_handler([](const httplib::Request &,
  ...
  // queries)
  auto require_auth_full = [this](const auto &req, auto &res, std::string &u,
                                  int64_t &uid, std::string &tok) -> bool {
  ...
  auto require_auth = [&require_auth_full](const auto &req, auto &res,
                                           std::string &u,
                                           std::string &tok) -> bool {
    int64_t uid = 0;
  ...
  auto login = [this, &kSetCookieFmt](auto &req, auto &res) {
    ...
    if (redis_ && redis_->IsConnected() && !login_user.empty()) {
      if (redis_->GetInt("rate:block:" + login_user) >= 1) {
        res.status = 429;
        res.set_content("{\"success\":false,\"error\":\"Account temporarily "
                        "locked. Retry later.\"}",
                        "application/json");
        return;
      }
      if (redis_->GetInt("rate:login:" + login_user + ":total") >= 15) {
        ...
        res.status = 429;
        res.set_content("{\"success\":false,\"error\":\"Account locked due to "
                        "too many attempts.\"}",
                        "application/json");
        return;
      }
    ...
    if (redis_ && redis_->IsConnected() && !login_user.empty()) {
      ...
      } else if (result == RpcResult::TRANSPORT_FAILURE ||
                 result == RpcResult::AUTH_FAILURE) {
        int64_t c = redis_->IncrementWithTTL(
            "rate:login:" + login_user + ":total", 300);
        if (c >= 15)
          redis_->SetJSON("rate:block:" + login_user, "1", 1800);
      }
    }
    ...
    if (redis_ && redis_->IsConnected() && !reg_user.empty()) {
      if (result == RpcResult::SUCCESS) {
        redis_->DeleteKey("rate:login:" + reg_user + ":total");
                 ...
                 result == RpcResult::AUTH_FAILURE) {
        int64_t c =
            redis_->IncrementWithTTL("rate:login:" + reg_user + ":total", 300);
        if (c >= 15)
          redis_->SetJSON("rate:block:" + reg_user, "1", 1800);
      }
    }
  ...
  auto refresh_tok = [this](auto &req, auto &res) {
    ...
    if (!st.ok() || !rresp.success()) {
      res.status = 401;
      res.set_content(R"({"error":"Invalid refresh token"})",
                      "application/json");
      return;
    }
    char buf[2048];
    snprintf(buf, sizeof(buf), kSetCookieFmt.c_str(),
             rresp.access_token().c_str());
    res.set_header("Set-Cookie", buf);
  ...
  auto file_up = [this, &require_auth_full](auto &req, auto &res) {
    ...
    if (file.filename.empty()) {
      res.set_content("{\"success\":false,\"error\":\"No file\"}",
                      "application/json");
      res.status = 400;
      return;
    }
    ...
    if (!st.ok() || !fresp.success()) {
      ...
      res.set_content("{\"success\":false,\"error\":\"Upload failed\"}",
                      "application/json");
      return;
    }
    cb_file_.RecordResult(true, elapsed);
    rep_file_.RecordSuccess(peer);
    res.set_content("{\"success\":true,\"id\":\"" + std::to_string(fresp.id()) +
                        "\"}",
                    "application/json");
  ...
  auto with_cb = [this](CircuitBreaker &cb, auto inner) {
    return [this, &cb, inner = std::move(inner)](auto &req, auto &res) {
      ...
      if (!cb.AllowRequest()) {
        ...
        res.set_content(
            R"({"error":"service unavailable","reason":"circuit_breaker_open"})",
            "application/json");
        return;
      }
  ...
  auto sh_create = [this](auto &req, std::string &r) {
    ...
    return HandleSheetCreate(u, uid, req.body,
                             req.get_header_value("X-Idempotency-Key"), tok,
                             rep_sheet_, r);
  };
  auto sh_list = [this](auto &req, std::string &r) {
    ...
    std::string p = req.get_param_value("page"),
                ps = req.get_param_value("page_size");
  ...
  auto me = [this](auto &req, auto &res) {
    ...
    std::string role = GetRoleFromCookie(req.get_header_value("Cookie"));
    res.set_content("{\"username\":" + JsonStr(u) +
                        ",\"role\":" + JsonStr(role) + "}",
                    "application/json");
  };
  svr.Get("/api/me", me);
  ...
  http_svr->Get("/api/breaker/stats",
                [&require_admin, this](auto &req, auto &res) {
                  if (!require_admin(req, res))
                    return;
                  std::string r;
                  HandleBreakerStats(r);
  ...
  http_svr->Post("/api/search",
                 [this, &require_auth_full](auto &req, auto &res) {
                   std::string u, tok;
                   int64_t uid = 0;
                   if (!require_auth_full(req, res, u, uid, tok))
                     return;
  ...
  if (!redis_cluster_seeds_.empty()) {
    redis_ = std::make_unique<RedisClient>(redis_cluster_seeds_,
                                           redis_password_, redis_pool_size_);
    if (redis_->Connect()) {
      ...
      printf("[Gateway] Circuit breakers connected to Redis (shared state "
             "enabled)\n");
    }
  }
