...
L1Cache::L1Cache(size_t max_entries, int ttl_seconds) : max_entries_(max_entries), ttl_seconds_(ttl_seconds) {}
...
void L1Cache::Set(const std::string &key, const std::string &value) {
    ...
    auto it = index_.find(key);
    if (it != index_.end()) {
        // Update existing â€?move to front
    ...
    lru_.emplace_front(Entry{key, value, std::chrono::steady_clock::now() + std::chrono::seconds(ttl_seconds_)});
...
size_t L1Cache::HitCount() const {
    return hits_;
}
size_t L1Cache::MissCount() const {
    return misses_;
}
