...
public:
  ...
  Database(const std::string &write_host, int write_port,
           const std::string &read_hosts, int read_port,
           const std::string &user, const std::string &password,
           const std::string &db_name, int write_pool_size = 4);
  ~Database();
  ...
  bool ListSpreadsheets(int64_t user_id, std::vector<SpreadsheetSummary> &out,
                        int &total, int page = 0, int page_size = 0);
  bool UpdateSpreadsheet(int64_t id, int64_t user_id, const std::string &name,
  ...
  // optimistic locking.
  bool GetSpreadsheetOwner(int64_t id, int64_t &owner_user_id,
                           int *out_version = nullptr);
  ...
  bool UpdateSpreadsheetStoragePath(int64_t id,
  ...
  MYSQL *GetConnection() {
    return write_conns_.empty() ? nullptr : write_conns_[0]->conn;
  }
...
private:
  ...
  bool ExecRead(const std::string &sql,
...
public:
  ...
  ShardedDatabase(int shard_count, const std::string &write_host_prefix,
                  int write_port, const std::string &read_hosts_prefix,
                  int read_port, const std::string &user,
                  const std::string &password,
                  const std::string &db_name_prefix, int write_pool_size = 4);
  ...
  bool UpdateSpreadsheetStoragePath(int64_t id,
                                    const std::string &storage_path);
  bool InsertOutbox(int64_t user_id, const std::string &event_type,
