...
grpc::Status SearchServiceImpl::Search(grpc::ServerContext*,
                                        const rpc::SearchRequest* req,
    ...
    if (s_sheets && s_files) indices = "sheets_search,files_search";
    else if (s_sheets) indices = "sheets_search";
    else if (s_files)  indices = "files_search";
    else { resp->set_total(0); return grpc::Status::OK; }
    ...
    if (s_sheets && s_files)
        mm["multi_match"]["fields"] = nlohmann::json::array({"name^3","description^2","cell_content","original_name^3","content_text"});
    else if (s_sheets)
        ...
        for (auto& hit : sr["hits"]["hits"]) {
            ...
            if (hit.contains("highlight")) {
                ...
                for (auto& [k, v] : hit["highlight"].items())
                    if (v.is_array() && !v.empty()) hl += v[0].get<std::string>() + " ";
                if (!hl.empty()) r->set_highlight(hl);
            }
        }
