...
RpcResult Gateway::HandleLogin(const std::string& body, std::string& response,
    ...
    if (ok) {
        ...
        nlohmann::json r;
        r["success"] = true; r["username"] = u;
        r["_rt"] = rt_out; r["_role"] = resp.role();
        response = r.dump();
    } else {
...
RpcResult Gateway::HandleSheetCreate(const std::string& username, int64_t user_id,
    ...
    if (ok) {
        ...
        char buf[32]; std::strftime(buf, sizeof(buf), "%Y-%m-%dT%H:%M:%SZ", std::gmtime(&now_t));
...
std::string Gateway::HandleSearch(const std::string& username, int64_t user_id,
    ...
    if (es_host_.empty()) return R"({"error":"Search service not configured"})";
    ...
    if (q.empty()) return R"({"error":"Query 'q' is required"})";
    ...
    std::string indices;
    if (search_sheets && search_files) indices = "sheets_search,files_search";
    else if (search_sheets)            indices = "sheets_search";
    else if (search_files)             indices = "files_search";
    else                               return R"({"total":0,"results":[]})";
    ...
    if (search_sheets && search_files) {
        multi_match["multi_match"]["fields"] = nlohmann::json::array(
            {"name^3", "description^2", "cell_content", "original_name^3", "content_text"}
        );
    } else if (search_sheets) {
        multi_match["multi_match"]["fields"] = nlohmann::json::array(
            {"name^3", "description^2", "cell_content"}
        );
    } else {
        multi_match["multi_match"]["fields"] = nlohmann::json::array(
    ...
    if (sort_by == "date_desc") {
        es_query["sort"] = nlohmann::json::array(
            {{{"updated_at", "desc"}}, {{"created_at", "desc"}}, {{"_score", "desc"}}}
        );
    } else if (sort_by == "date_asc") {
        es_query["sort"] = nlohmann::json::array(
            {{{"updated_at", "asc"}}, {{"created_at", "asc"}}, {{"_score", "desc"}}}
        );
    }
    ...
    auto es_res = es_cli.Post("/" + indices + "/_search",
    ...
    try {
        ...
        int64_t total = sr.value("hits", nlohmann::json::object()).value("total", nlohmann::json::object()).value("value", 0);
...
void Gateway::IndexToES(const std::string& index_name, const std::string& doc_id,
                         const std::string& json_body) const {
    if (es_host_.empty()) return;
    std::thread([host = es_host_, idx = index_name, id = doc_id, body = json_body]() {
        ...
        if (!res || (res->status != 200 && res->status != 201)) {
            fprintf(stderr, "[ES] index %s/%s failed: %s\n",
                    idx.c_str(), id.c_str(), res ? res->body.c_str() : "no response");
        }
    }).detach();
}
...
void Gateway::NotifyDocParser(const std::string& endpoint, const std::string& json_body) const {
    if (doc_parser_host_.empty()) return;
...
bool Gateway::Start() {
    ...
    auto refresh_tok = [this](auto& req, auto& res) {
        ...
        rpc::RefreshTokenRequest rreq; rpc::RefreshTokenResponse rresp;
        rreq.set_username(u); rreq.set_refresh_token(rt);
        grpc::ClientContext ctx; ctx.set_deadline(std::chrono::system_clock::now() + std::chrono::seconds(2));
        ...
        char buf[2048]; snprintf(buf, sizeof(buf), kSetCookieFmt.c_str(), rresp.access_token().c_str());
        res.set_header("Set-Cookie", buf);
