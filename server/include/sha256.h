...
inline std::string hash_password(const std::string &password, int iterations = 100000) {
    unsigned char salt[16];
    if (!RAND_bytes(salt, sizeof(salt))) {
        // RAND_bytes 失败时回退到裸 SHA256（极度罕见）
        fprintf(stderr, "[sha256] RAND_bytes failed, falling back to bare SHA256\n");
        return "sha256$1$$" + hash_hex(password);
    }

    unsigned char key[32];
    PKCS5_PBKDF2_HMAC(password.data(), (int)password.size(), salt, (int)sizeof(salt), iterations, EVP_sha256(),
                      (int)sizeof(key), key);
...
inline bool verify_password(const std::string &password, const std::string &stored) {
    ...
    int salt_len = 0;
    for (size_t i = 0; i + 1 < salt_hex.size() && salt_len < (int)sizeof(salt); i += 2)
        salt[salt_len++] = (unsigned char)std::stoi(salt_hex.substr(i, 2), nullptr, 16);

    // hex hash → bytes
    unsigned char expected[64];
    int expected_len = 0;
    for (size_t i = 0; i + 1 < hash_hex_stored.size() && expected_len < (int)sizeof(expected); i += 2)
        expected[expected_len++] = (unsigned char)std::stoi(hash_hex_stored.substr(i, 2), nullptr, 16);
    ...
    PKCS5_PBKDF2_HMAC(password.data(), (int)password.size(), salt, salt_len, iterations, EVP_sha256(), computed_len,
                      computed);
