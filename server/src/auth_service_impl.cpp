...
std::string AuthServiceImpl::UsersFilePath() const { return "users.json"; }
...
grpc::Status AuthServiceImpl::Login(grpc::ServerContext *,
  ...
  if (slog_)
    LOG_INFO(*slog_, std::string("User '") + req->username() +
                         "' logged in (ver=" + std::to_string(ver) + ")");
  return grpc::Status::OK;
}
...
grpc::Status AuthServiceImpl::Register(grpc::ServerContext *,
  ...
  if (req->username().size() < 3 || req->username().size() > 20) {
    ...
    if (logger_) {
      ...
      logger_->Log(req->username(), "AuthService", "Register", p, r, false,
                   dur);
    }
    return grpc::Status::OK;
  }
  ...
  if (db_) {
    ...
    if (!db_->AddUser(req->username(),
                      sha256::hash_password(req->password()))) {
      bool is_dup = db_->UserExists(req->username());
      resp->set_success(false);
      resp->set_error(is_dup
      ...
      if (logger_) {
        ...
        p["username"] = json(req->username());
        r["error"] =
            json(is_dup ? "Username already exists" : "Registration failed");
        logger_->Log(req->username(), "AuthService", "Register", p, r, false,
                     dur);
      }
      return grpc::Status::OK;
...
std::string AuthServiceImpl::CreateAccessToken(const std::string &username,
                                               int64_t uid,
...
std::string AuthServiceImpl::CreateRefreshToken(const std::string &username,
                                                int64_t uid) const {
  ...
  std::string rt(buf);
  if (redis_ && redis_->IsConnected())
    redis_->SetJSON("rt:" + username,
                    "{\"token\":\"" + rt + "\",\"uid\":" + std::to_string(uid) +
                        "}",
                    604800);
  return rt;
}

grpc::Status AuthServiceImpl::RefreshToken(grpc::ServerContext *,
...
grpc::Status AuthServiceImpl::ValidateUser(grpc::ServerContext *,
