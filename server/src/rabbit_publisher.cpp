...
bool RabbitPublisher::Connect() {
    ...
    if (!socket_) { fprintf(stderr, "[RabbitMQ] socket create failed\n"); return false; }
    int st = amqp_socket_open(socket_, host_.c_str(), port_);
    if (st) { fprintf(stderr, "[RabbitMQ] connect failed\n"); return false; }
    amqp_rpc_reply_t reply = amqp_login(conn_, "/", 0, 131072, 0,
                                         AMQP_SASL_METHOD_PLAIN, user_.c_str(), pass_.c_str());
...
bool RabbitPublisher::Publish(const std::string& exchange, const std::string& routing_key,
