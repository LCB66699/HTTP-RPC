...
AuthContext AuthInterceptor::FromToken(const std::string &token, const std::string &jwt_secret) {
    ...
    // 妫€鏌?JWT 杩囨湡鏃堕棿锛坋xp 瀛楁锛孶nix 鏃堕棿鎴筹級
    auto exp_pos = payload.find("\"exp\"");
    if (exp_pos != std::string::npos) {
        auto colon = payload.find(':', exp_pos + 5);
        if (colon != std::string::npos) {
            size_t num_start = colon + 1;
            while (num_start < payload.size() && (payload[num_start] == ' ' || payload[num_start] == '\t'))
                num_start++;
            size_t num_end = num_start;
            while (num_end < payload.size() && payload[num_end] >= '0' && payload[num_end] <= '9')
                num_end++;
