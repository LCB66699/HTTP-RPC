...
public:
  ...
  bool PushCallEntry(const std::string &json_entry,
                     const std::string &username = "");
  std::vector<std::string>
  GetCallEntries(int limit, int offset, const std::string &username = "") const;
  int64_t GetCallCount(const std::string &username = "") const;
  std::vector<std::string> GetHistoryUsers() const;

  // Cache writes
  bool SetJSON(const std::string &key, const std::string &value,
               int ttl_seconds);
  ...
  bool HSetJSON(const std::string &key, const std::string &field,
  ...
  using SubCallback =
      std::function<void(const std::string &channel, const std::string &msg)>;
