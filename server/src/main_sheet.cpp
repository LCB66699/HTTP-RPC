...
int main(int argc, char *argv[]) {
  ...
  auto db = std::make_unique<ShardedDatabase>(
      mysql_shards, mysql_host, mysql_port, mysql_host, mysql_port, mysql_user,
      mysql_password, mysql_db, 4);
  db->InitializeAll();
  db->StartHealthCheck();

  std::unique_ptr<RedisClient> redis;
  if (!redis_cluster_seeds.empty()) {
    redis = std::make_unique<RedisClient>(redis_cluster_seeds, redis_password,
                                          redis_pool_size);
    redis->Connect();
  }
  ...
  auto l1_invalidator =
      std::make_unique<L1CacheInvalidator>(l1_cache.get(), redis.get());
  l1_invalidator->Start();

  std::unique_ptr<RabbitPublisher> rabbit_pub;
  const char *rb_host = std::getenv("RABBITMQ_HOST");
  if (rb_host)
    rabbit_pub = std::make_unique<RabbitPublisher>(rb_host, 5672, "rpc",
                                                   "rpc-rabbit-123456");
  ...
  if (minio_ep && minio_ak && minio_sk && minio_bk) {
    ...
    minio_client.public_url =
        minio_pub ? minio_pub : ("http://" + std::string(minio_ep));
  ...
  health_monitor.SetNodeInfo("sheet-" + std::to_string(port), "sheet", host,
                             port);
  ...
  sheet_service.SetAuthChannel(grpc::CreateCustomChannel(
