...
基于 gRPC + gRPC-Gateway 的在线协作表格系统，支持文件存储、工作空间、实时协作、多层缓存、服务发现、弹性容错和可观测性。
...
nginx :443                              ← TLS 终结 + 令牌桶限流
   ...
   ▼
Envoy ×2 :10000                         ← JWT 边缘认证 + CORS + 熔断 + 重试
   │  STRICT_DNS → gateway_cluster
   ▼
gRPC-Gateway (Go) ×2 :8080             ← HTTP/JSON ↔ gRPC + JWT + 熔断器 + WebSocket Hub
   │  consul:/// (Consul 服务发现)
   ├──→ Auth/Sharing/Workspace (rpc-auth:50051)  ×2   MySQL rpc_auth
