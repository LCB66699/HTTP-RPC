...
grpc::Status SearchServiceImpl::Search(grpc::ServerContext *,
  ...
  if (req->sort() == "date_desc")
    esq["sort"] = nlohmann::json::array({{{"updated_at", "desc"}},
                                         {{"created_at", "desc"}},
                                         {{"_score", "desc"}}});
  else if (req->sort() == "date_asc")
    esq["sort"] = nlohmann::json::array({{{"updated_at", "asc"}},
                                         {{"created_at", "asc"}},
                                         {{"_score", "desc"}}});

  esq["highlight"]["fields"]["*"] = {{"fragment_size", 80},
  ...
  auto es_res =
      es.Post("/" + indices + "/_search", esq.dump(), "application/json");
