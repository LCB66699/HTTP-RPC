...
grpc::Status SpreadsheetServiceImpl::CreateSpreadsheet(
...

    {
        ...
        if (rabbit_) rabbit_->Publish("rpc.events", "sheet.created", ev.dump());
        if (db_) db_->InsertOutbox(req->user_id(), "sheet.created", ev.dump());
    }
    ...
    for (int attempt = 0; attempt < MAX_RETRIES; ++attempt) {
        ...
        if (ok) {
...

            {
                ...
                if (rabbit_) rabbit_->Publish("rpc.events", "sheet.updated", ev.dump());
                if (db_) db_->InsertOutbox(req->user_id(), "sheet.updated", ev.dump());
            }
...
grpc::Status SpreadsheetServiceImpl::DeleteSpreadsheet(
...

    {
        ...
        ev["id"] = req->id();
        ev["user_id"] = req->user_id();
        if (rabbit_) rabbit_->Publish("rpc.events", "sheet.deleted", ev.dump());
        if (db_) db_->InsertOutbox(req->user_id(), "sheet.deleted", ev.dump());
    }
