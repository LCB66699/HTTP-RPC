...
inline std::string hmac_sha256(const std::string &data,
...
inline std::string create(const std::string &payload_json,
                          const std::string &secret) {
  std::string header = base64url_encode("{\"alg\":\"HS256\",\"typ\":\"JWT\"}");
  std::string payload = base64url_encode(payload_json);
  std::string sig =
      base64url_encode(hmac_sha256(header + "." + payload, secret));
  return header + "." + payload + "." + sig;
}

inline bool verify(const std::string &token, const std::string &secret,
