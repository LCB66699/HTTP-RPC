...
private:
    ...
    std::string es_host_;  // Elasticsearch REST API URL
    std::string doc_parser_host_ = "http://doc-parser:9002";
    ...
    // ES 全文搜索
    std::string HandleSearch(const std::string& username, int64_t user_id,
                              const std::string& body) const;
    void IndexToES(const std::string& index_name, const std::string& doc_id,
                    const std::string& json_body) const;
    void NotifyDocParser(const std::string& endpoint, const std::string& json_body) const;
