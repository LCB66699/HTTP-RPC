...
struct FileRow {
    ...
    std::string storage_path;   // non-empty → content lives in object storage, not file_content
};
...
public:
    ...
    // username is stored alongside for display; idempotency_key deduplicates retries.
    ...
    // storage_key: object-storage path returned by MinIO PutObject (empty = legacy BLOB mode).
    ...
    // Delete undo_log entries older than `days` days; call from a maintenance goroutine/timer
