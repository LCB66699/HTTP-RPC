...
std::string AuthServiceImpl::CreateAccessToken(const std::string& username, int64_t uid, const std::string& role) const {
...
grpc::Status AuthServiceImpl::RefreshToken(grpc::ServerContext*,
    ...
    if (stored_rt != req->refresh_token()) {
        // Token 不匹配 → 盗用检测，撤销所有
        if (redis_) { redis_->DeleteKey("rt:" + req->username()); redis_->DeleteKey("rate:login:" + req->username() + ":total"); }
        resp->set_success(false);
        resp->set_error("Invalid refresh token");
        return grpc::Status::OK;
    }
...
grpc::Status AuthServiceImpl::ValidateUser(grpc::ServerContext*,
    ...
    if (db_) {
        ...
        if (uid == 0) uid = db_->GetUserId(username);
    } else {
