...
AuthInterceptor::AuthInterceptor(const std::string &jwt_secret)
...
AuthContext AuthInterceptor::Authenticate(grpc::ServerContext *ctx) {
  ...
  std::string payload;
  fprintf(stderr,
          "[Auth] verify: secret_len=%zu token_len=%zu token_head=%.20s\n",
  ...
  if (exp_pos != std::string::npos) {
    ...
    if (colon != std::string::npos) {
      ...
      while (num_start < payload.size() &&
             (payload[num_start] == ' ' || payload[num_start] == '\t'))
        num_start++;
      size_t num_end = num_start;
      while (num_end < payload.size() && payload[num_end] >= '0' &&
             payload[num_end] <= '9')
        num_end++;
  ...
  if (uid_pos != std::string::npos) {
    ...
    if (colon2 != std::string::npos) {
      ...
      if (num_end > num_start)
        auth.user_id =
            std::stoll(payload.substr(num_start, num_end - num_start));
    }
  }
